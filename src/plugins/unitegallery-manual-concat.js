...
function debugLine(html, addRandom, addHtml) {
    ...
    if (addHtml == true) {
        const objLine = jQuery('#debug_line');
        objLine.width(200);

        if (objLine.height() >= 500) { objLine.html(''); }

        const currentHtml = objLine.html();
...
function UGFunctions() {
    ...
    this.getBrowserPrefix = function () {
        ...
        const arrayOfPrefixes = ['webkit', 'Moz', 'ms', 'O'];

        const div = document.createElement('div');

        for (const index in arrayOfPrefixes) {
            let prefix = arrayOfPrefixes[index];
    ...
    this.getImageInsideParentDataByImage = function (objImage, scaleMode, objPadding) {
        const objParent = objImage.parent();

        const objOrgSize = t.getImageOriginalSize(objImage);

        const objData = t.getImageInsideParentData(objParent, objOrgSize.width, objOrgSize.height, scaleMode, objPadding);
    ...
    this.getImageInsideParentData = function (objParent, originalWidth, originalHeight, scaleMode, objPadding, maxWidth, maxHeight) {
        ...
        let imageTop = null;
        let imageLeft = null;
        let style = 'display:block;margin:0px auto;';
    ...
    this.getElementCenterPosition = function (element, objPadding) {
        ...
        const objSize = t.getElementSize(element);
        const objSizeParent = t.getElementSize(parent);

        let parentWidth = objSizeParent.width;
        let parentHeight = objSizeParent.height;
    ...
    this.getMouseElementPoint = function (point, element) {
        ...
        const newPoint = { x: point.pageX, y: point.pageY };

        const elementPoint = t.getElementLocalPoint(newPoint, element);
    ...
    this.getImageOriginalSize = function (objImage, originalWidth, originalHeight) {
        if (typeof originalWidth !== 'undefined' && typeof originalHeight !== 'undefined') { return ({ width: originalWidth, height: originalHeight }); }

        const htmlImage = objImage[0];
    ...
    this.getimageRatio = function (objImage) {
        const originalSize = t.getImageOriginalSize(objImage);
        const size = t.getElementSize(objImage);
        const ratio = size.width / originalSize.width;
    ...
    this.isImageFitParent = function (objImage) {
        const objParent = objImage.parent();
        const sizeImage = t.getElementSize(objImage);
        const sizeParent = t.getElementSize(objParent);
    ...
    this.isElementBiggerThenParent = function (element) {
        const objParent = element.parent();
        const objSizeElement = t.getElementSize(element);
        const objSizeParent = t.getElementSize(objParent);
    ...
    this.isPointInsideElement = function (point, objSize) {
        const isMouseXInside = (point.x >= 0 && point.x < objSize.width);
        if (isMouseXInside == false) { return (false); }

        const isMouseYInside = (point.y >= 0 && point.y < objSize.height);
    ...
    this.getElementRelativePos = function (element, pos, offset, objParent) {
        ...
        const parentSize = t.getElementSize(objParent);
    ...
    this.zoomImageInsideParent = function (objImage, zoomIn, step, point, scaleMode, maxZoomRatio, objPadding) {
        ...
        const zoomRatio = step;

        const objParent = objImage.parent();

        const objSize = t.getElementSize(objImage);
        const objOriginalSize = t.getImageOriginalSize(objImage);
        ...
        if (!point) {
            ...
            const pointImg = t.getMouseElementPoint(point, objImage);
        ...
        if (isMouseInside == false) {
            const imgCenterPoint = t.getElementCenterPoint(objImage);
        ...
        if (zoomIn == true) {		// zoom in
            ...
            const objScaleData = t.getImageInsideParentData(objParent, objOriginalSize.width, objOriginalSize.height, scaleMode, objPadding);
        ...
        if (zoomIn == false && isMouseInside == false) {
            const posCenter = t.getElementCenterPosition(objImage);
            newX = posCenter.left;
            newY = posCenter.top;
        } else {
    ...
    this.cloneElementSizeAndPos = function (objSource, objTarget, isOuter, offsetX, offsetY) {
        const objSize = objSource.position();
    ...
    this.placeImageInsideParent = function (urlImage, objParent, originalWidth, originalHeight, scaleMode, objPadding) {
        const obj = t.getImageInsideParentData(objParent, originalWidth, originalHeight, scaleMode, objPadding);
        ...
        const objImage = objParent.children('img');
    ...
    this.scaleImageCoverParent = function (objImage, objParent, pHeight) {
        ...
        const imageWidth = objOriginalSize.width;
        const imageHeight = objOriginalSize.height;

        const ratio = imageWidth / imageHeight;

        let fitHeight = parentHeight;
        let fitWidth = fitHeight * ratio;
        let posy = 0,
            posx = 0;
    ...
    this.scaleImageByHeight = function (objImage, height, originalWidth, originalHeight) {
        const objOriginalSize = t.getImageOriginalSize(objImage, originalWidth, originalHeight);

        const ratio = objOriginalSize.width / objOriginalSize.height;
    ...
    this.scaleImageExactSizeInParent = function (objImage, originalWidth, originalHeight, exactWidth, exactHeight, scaleMode) {
        ...
        const obj = t.getImageInsideParentData(null, originalWidth, originalHeight, scaleMode, null, exactWidth, exactHeight);

        const imageWidth = exactWidth;
        const imageHeight = exactHeight;

        const paddingLeft = obj.imageLeft;
        let paddingRight = obj.imageLeft;
        const paddingTop = obj.imageTop;
        let paddingBottom = obj.imageTop;
        const imageLeft = Math.round((parentSize.width - exactWidth) / 2);
        const imageTop = Math.round((parentSize.height - exactHeight) / 2);

        const totalWidth = obj.imageWidth + paddingLeft + paddingRight;
        var diff = exactWidth - totalWidth;
        if (diff != 0) { paddingRight += diff; }

        const totalHeight = obj.imageHeight + paddingTop + paddingBottom;
    ...
    this.convertCustomPrefixOptions = function (objOptions, prefix, base) {
        ...
        jQuery.each(objOptions, (key, option) => {
            if (key.indexOf(`${prefix}_${base}_`) === 0) {
                const newKey = key.replace(`${prefix}_${base}_`, `${base}_`);
    ...
    this.normalizeSetting = function (realMin, realMax, settingMin, settingMax, value, fixBounds) {
        ...
        const ratio = (value - settingMin) / (settingMax - settingMin);
    ...
    this.getNormalizedValue = function (realMin, realMax, settingMin, settingMax, realValue) {
        const ratio = (realValue - realMin) / (realMax - realMin);
    ...
    this.getDistance = function (x1, y1, x2, y2) {
        const distance = Math.round(Math.sqrt(Math.abs(((x2 - x1) * (x2 - x1)) + ((y2 - y1) * (y2 - y1)))));
    ...
    this.getNumItemsInSpace = function (spaceSize, itemsSize, gapSize) {
        const numItems = Math.floor((spaceSize + gapSize) / (itemsSize + gapSize));
        return (numItems);
    };
    ...
    this.getNumItemsInSpaceRound = function (spaceSize, itemsSize, gapSize) {
        const numItems = Math.round((spaceSize + gapSize) / (itemsSize + gapSize));
        return (numItems);
    };
    ...
    this.getSpaceByNumItems = function (numItems, itemSize, gapSize) {
        const space = numItems * itemSize + (numItems - 1) * gapSize;
        return (space);
    };
    ...
    this.getItemSizeInSpace = function (spaceSize, numItems, gapSize) {
        const itemSize = Math.floor((spaceSize - (numItems - 1) * gapSize) / numItems);
    ...
    this.getColX = function (col, colWidth, colGap) {
        const posx = col * (colWidth + colGap);
    ...
    this.getPrevRowSameColIndex = function (index, numCols) {
        const obj = t.getColRowByIndex(index, numCols);
        const prevIndex = t.getIndexByRowCol(obj.row - 1, obj.col, numCols);
        return (prevIndex);
    };
    ...
    this.getNextRowSameColIndex = function (index, numCols) {
        const obj = t.getColRowByIndex(index, numCols);
        const nextIndex = t.getIndexByRowCol(obj.row + 1, obj.col, numCols);
        return (nextIndex);
    };
    ...
    this.getGlobalData = function (key) {
        const value = jQuery.data(document.body, key);
    ...
    this.handleScrollTop = function (storedEventID) {
        ...
        const objData = t.getStoredEventData(storedEventID);
        ...
        if (objData.scrollDir === null) {
            if (Math.abs(objData.diffMouseX) > horPass) { objData.scrollDir = 'hor'; } else
        ...
        const scrollPos = objData.scrollOrigin - (objData.lastMouseClientY - objData.scrollStartY);
    ...
    this.storeEventData = function (event, id, addData) {
        const mousePos = t.getMousePosition(event);
        const time = jQuery.now();
    ...
    this.isApproveStoredEventClick = function (id, isVertical) {
        ...
        const objData = t.getStoredEventData(id, isVertical);

        const passedDistanceAbs = Math.abs(objData.diffMousePos);
    ...
    this.isCanvasExists = function () {
        const canvas = jQuery('<canvas width="500" height="500" > </canvas>')[0];
    ...
    this.isRgbaSupported = function () {
        ...
        const result = scriptElement.style.color != prevColor;
    ...
    this.checkMinJqueryVersion = function (version) {
        const arrCurrent = jQuery.fn.jquery.split('.');
        const arrMin = version.split('.');

        for (let i = 0, len = arrCurrent.length; i < len; i++) {
            const numCurrent = parseInt(arrCurrent[i]);
            const numMin = parseInt(arrMin[i]);
    ...
    this.getArrTouchPositions = function (arrTouches) {
        ...
        for (let i = 0; i < arrTouches.length; i++) {
            const point = {
    ...
    this.showTimeDebug = function () {
        const endTime = jQuery.now();
        const diffTime = endTime - g_temp.starTime;
    ...
    this.setButtonMobileReady = function (objButton) {
        ...
        objButton.on('mousedown touchend', (event) => {
    ...
    this.getFirstRegisteredTheme = function () {
        ...
        const themeName = g_temp.arrThemes[0];
    ...
    this.validateClickTouchstartEvent = function (eventType) {
        ...
        const diff = jQuery.now() - g_temp.lastEventTime;

        // debugLine({lastType:g_temp.lastEventType, diff:diff},true, true);

        if (eventType == 'click' && g_temp.lastEventType == 'touchstart' && diff < 1000) { returnVal = false; }
    ...
    this.checkImagesLoaded = function (objImages, onComplete, onProgress) {
        const arrImages = [];
        let counter = 0;
        const numImages = objImages.length;
        ...
        setTimeout(() => {
            ...
            for (let index = 0; index < numImages; index++) {
                const image = objImages[index];

                // arrImages.push(jQuery(image));
                if (image.naturalWidth !== undefined && image.naturalWidth !== 0) {
                    checkComplete(objImages[index], false);
                } else {
                    const proxyImage = jQuery('<img/>');
    ...
    this.htmlentitles = function (html) {
        const text = jQuery('<div/>').text(html).html();
        return (text);
    };
...
const g_ugFunctions = new UGFunctions();
...
!(function (e) { typeof define === 'function' && define.amd ? define(['jquery'], e) : typeof exports === 'object' ? module.exports = e : e(jQuery); }((e) => {
    function t(t) {
        let s = t || window.event,
            a = h.call(arguments, 1),
            u = 0,
            r = 0,
            d = 0,
            f = 0; if (t = e.event.fix(s), t.type = 'mousewheel', 'detail' in s && (d = -1 * s.detail), 'wheelDelta' in s && (d = s.wheelDelta), 'wheelDeltaY' in s && (d = s.wheelDeltaY), 'wheelDeltaX' in s && (r = -1 * s.wheelDeltaX), 'axis' in s && s.axis === s.HORIZONTAL_AXIS && (r = -1 * d, d = 0), u = d === 0 ? r : d, 'deltaY' in s && (d = -1 * s.deltaY, u = d), 'deltaX' in s && (r = s.deltaX, d === 0 && (u = -1 * r)), d !== 0 || r !== 0) { if (s.deltaMode === 1) { const c = e.data(this, 'mousewheel-line-height'); u *= c, d *= c, r *= c; } else if (s.deltaMode === 2) { const m = e.data(this, 'mousewheel-page-height'); u *= m, d *= m, r *= m; } return f = Math.max(Math.abs(d), Math.abs(r)), (!l || l > f) && (l = f, i(s, f) && (l /= 40)), i(s, f) && (u /= 40, r /= 40, d /= 40), u = Math[u >= 1 ? 'floor' : 'ceil'](u / l), r = Math[r >= 1 ? 'floor' : 'ceil'](r / l), d = Math[d >= 1 ? 'floor' : 'ceil'](d / l), t.deltaX = r, t.deltaY = d, t.deltaFactor = l, t.deltaMode = 0, a.unshift(t, u, r, d), o && clearTimeout(o), o = setTimeout(n, 200), (e.event.dispatch || e.event.handle).apply(this, a); }
    } function n() { l = null; } function i(e, t) { return r.settings.adjustOldDeltas && e.type === 'mousewheel' && t % 120 === 0; } var o,
        l,
        s = ['wheel', 'mousewheel', 'DOMMouseScroll', 'MozMousePixelScroll'],
        a = 'onwheel' in document || document.documentMode >= 9 ? ['wheel'] : ['mousewheel', 'DomMouseScroll', 'MozMousePixelScroll'],
        h = Array.prototype.slice; if (e.event.fixHooks) for (let u = s.length; u;)e.event.fixHooks[s[--u]] = e.event.mouseHooks; var r = e.event.special.mousewheel = { version: '3.1.9', setup() { if (this.addEventListener) for (let n = a.length; n;) this.addEventListener(a[--n], t, !1); else this.onmousewheel = t; e.data(this, 'mousewheel-line-height', r.getLineHeight(this)), e.data(this, 'mousewheel-page-height', r.getPageHeight(this)); }, teardown() { if (this.removeEventListener) for (let e = a.length; e;) this.removeEventListener(a[--e], t, !1); else this.onmousewheel = null; }, getLineHeight(t) { return parseInt(e(t)['offsetParent' in e.fn ? 'offsetParent' : 'parent']().css('fontSize'), 10); }, getPageHeight(t) { return e(t).height(); }, settings: { adjustOldDeltas: !0 } }; e.fn.extend({ mousewheel(e) { return e ? this.bind('mousewheel', e) : this.trigger('mousewheel'); }, unmousewheel(e) { return this.unbind('mousewheel', e); } });
}));
...
(function (factory) { if (typeof define === 'function' && define.amd) { define(['jquery'], $ => factory($)); } else if (typeof module === 'object' && typeof module.exports === 'object') { exports = factory(require('jquery')); } else { factory(jQuery); } }(($) => {
    $.easing.jswing = $.easing.swing; let pow = Math.pow,
        sqrt = Math.sqrt,
        sin = Math.sin,
        cos = Math.cos,
        PI = Math.PI,
        ...
        let n1 = 7.5625,
            d1 = 2.75; if (x < 1 / d1) { return n1 * x * x; } else if (x < 2 / d1) { return n1 * (x -= 1.5 / d1) * x + 0.75; } else if (x < 2.5 / d1) { return n1 * (x -= 2.25 / d1) * x + 0.9375; } return n1 * (x -= 2.625 / d1) * x + 0.984375;
...
!(function (r, n) {
    ...
    { return (false); }
    function t(r, n, t) { const e = f[n.type] || {}; return r == null ? t || !n.def ? null : n.def : (r = e.floor ? ~~r : parseFloat(r), isNaN(r) ? n.def : e.mod ? (r + e.mod) % e.mod : r < 0 ? 0 : e.max < r ? e.max : r); } function e(n) {
        let t = l(),
            e = t._rgba = []; return n = n.toLowerCase(), h(u, (r, o) => {
            let a,
                s = o.re.exec(n),
                i = s && o.parse(s),
                u = o.space || 'rgba'; return i ? (a = t[u](i), t[c[u].cache] = a[c[u].cache], e = t._rgba = a._rgba, !1) : void 0;
        }), e.length ? (e.join() === '0,0,0,0' && r.extend(e, a.transparent), t) : a[n];
    } function o(r, n, t) { return t = (t + 1) % 1, 6 * t < 1 ? r + (n - r) * t * 6 : 2 * t < 1 ? n : 3 * t < 2 ? r + (n - r) * (2 / 3 - t) * 6 : r; } var a,
        ...
        p = l.support = {},
        d = r('<p>')[0],
        h = r.each; d.style.cssText = 'background-color:rgba(1,1,1,.5)', p.rgba = d.style.backgroundColor.indexOf('rgba') > -1, h(c, (r, n) => { n.cache = `_${r}`, n.props.alpha = { idx: 3, type: 'percent', def: 1 }; }), l.fn = r.extend(l.prototype, { parse(o, s, i, u) {
        if (o === n) return this._rgba = [null, null, null, null], this; (o.jquery || o.nodeType) && (o = r(o).css(s), s = n); let f = this,
            p = r.type(o),
            d = this._rgba = []; return s !== n && (o = [o, s, i, u], p = 'array'), p === 'string' ? this.parse(e(o) || a._default) : p === 'array' ? (h(c.rgba.props, (r, n) => { d[n.idx] = t(o[n.idx], n); }), this) : p === 'object' ? (o instanceof l ? h(c, (r, n) => { o[n.cache] && (f[n.cache] = o[n.cache].slice()); }) : h(c, (n, e) => { const a = e.cache; h(e.props, (r, n) => { if (!f[a] && e.to) { if (r === 'alpha' || o[r] == null) return; f[a] = e.to(f._rgba); }f[a][n.idx] = t(o[r], n, !0); }), f[a] && r.inArray(null, f[a].slice(0, 3)) < 0 && (f[a][3] = 1, e.from && (f._rgba = e.from(f[a]))); }), this) : void 0;
    },
    is(r) {
        let n = l(r),
            t = !0,
            e = this; return h(c, (r, o) => {
            let a,
                s = n[o.cache]; return s && (a = e[o.cache] || o.to && o.to(e._rgba) || [], h(o.props, (r, n) => (s[n.idx] != null ? t = s[n.idx] === a[n.idx] : void 0))), t;
        }), t;
    },
    _space() {
        let r = [],
            n = this; return h(c, (t, e) => { n[e.cache] && r.push(t); }), r.pop();
    },
    transition(r, n) {
        let e = l(r),
            o = e._space(),
            a = c[o],
            s = this.alpha() === 0 ? l('transparent') : this,
            i = s[a.cache] || a.to(s._rgba),
            u = i.slice(); return e = e[a.cache], h(a.props, (r, o) => {
            let a = o.idx,
                s = i[a],
                l = e[a],
                c = f[o.type] || {}; l !== null && (s === null ? u[a] = l : (c.mod && (l - s > c.mod / 2 ? s += c.mod : s - l > c.mod / 2 && (s -= c.mod)), u[a] = t((l - s) * n + s, o)));
        }), this[o](u);
    },
    blend(n) {
        if (this._rgba[3] === 1) return this; let t = this._rgba.slice(),
            e = t.pop(),
            o = l(n)._rgba; return l(r.map(t, (r, n) => (1 - e) * o[n] + e * r));
    },
    toRgbaString() {
        let n = 'rgba(',
            t = r.map(this._rgba, (r, n) => (r == null ? n > 2 ? 1 : 0 : r)); return t[3] === 1 && (t.pop(), n = 'rgb('), `${n + t.join()})`;
    },
    toHslaString() {
        let n = 'hsla(',
            t = r.map(this.hsla(), (r, n) => r == null && (r = n > 2 ? 1 : 0), n && n < 3 && (r = `${Math.round(100 * r)}%`), r); return t[3] === 1 && (t.pop(), n = 'hsl('), `${n + t.join()})`;
    },
    toHexString(n) {
        let t = this._rgba.slice(),
            e = t.pop(); return n && t.push(~~(255 * e)), `#${r.map(t, r => r = (r || 0).toString(16), r.length === 1 ? `0${r}` : r).join('')}`;
    },
    toString() { return this._rgba[3] === 0 ? 'transparent' : this.toRgbaString(); } }), l.fn.parse.prototype = l.fn, c.hsla.to = function (r) {
        if (r[0] == null || r[1] == null || r[2] == null) return [null, null, null, r[3]]; let n,
            t,
            e = r[0] / 255,
            o = r[1] / 255,
            a = r[2] / 255,
            ...
            l = i - u,
            c = i + u,
            f = 0.5 * c; return n = u === i ? 0 : e === i ? 60 * (o - a) / l + 360 : o === i ? 60 * (a - e) / l + 120 : 60 * (e - o) / l + 240, t = l === 0 ? 0 : f <= 0.5 ? l / c : l / (2 - c), [Math.round(n) % 360, t, f, s == null ? 1 : s];
    }, c.hsla.from = function (r) {
        if (r[0] == null || r[1] == null || r[2] == null) return [null, null, null, r[3]]; let n = r[0] / 360,
            t = r[1],
            e = r[2],
            a = r[3],
            s = e <= 0.5 ? e * (1 + t) : e + t - e * t,
            i = 2 * e - s; return [Math.round(255 * o(i, s, n + 1 / 3)), Math.round(255 * o(i, s, n)), Math.round(255 * o(i, s, n - 1 / 3)), a];
    }, h(c, (e, o) => {
        let a = o.props,
            s = o.cache,
            u = o.to,
            c = o.from; l.fn[e] = function (e) {
            if (u && !this[s] && (this[s] = u(this._rgba)), e === n) return this[s].slice(); let o,
                i = r.type(e),
                f = i === 'array' || i === 'object' ? e : arguments,
                p = this[s].slice(); return h(a, (r, n) => { let e = f[i === 'object' ? r : n.idx]; e == null && (e = p[n.idx]), p[n.idx] = t(e, n); }), c ? (o = l(c(p)), o[s] = p, o) : l(p);
        }, h(a, (n, t) => {
            l.fn[n] || (l.fn[n] = function (o) {
                let a,
                    ...
                    l = this[u](),
                    c = l[t.idx]; return s === 'undefined' ? c : (s === 'function' && (o = o.call(this, c), s = r.type(o)), o == null && t.empty ? this : (s === 'string' && (a = i.exec(o), a && (o = c + parseFloat(a[2]) * (a[1] === '+' ? 1 : -1))), l[t.idx] = o, this[u](l)));
            });
        });
    }), l.hook = function (n) {
        const t = n.split(' '); h(t, (n, t) => {
            r.cssHooks[t] = { set(n, o) {
                let a,
                    s,
                    i = ''; if (o !== 'transparent' && (r.type(o) !== 'string' || (a = e(o)))) { if (o = l(a || o), !p.rgba && o._rgba[3] !== 1) { for (s = t === 'backgroundColor' ? n.parentNode : n; (i === '' || i === 'transparent') && s && s.style;) try { i = r.css(s, 'backgroundColor'), s = s.parentNode; } catch (u) {}o = o.blend(i && i !== 'transparent' ? i : '_default'); }o = o.toRgbaString(); } try { n.style[t] = o; } catch (u) {}
 ...
 */
!(function (t) {
    function e() { try { var i = this === document ? t(this) : t(this).contents(); } catch (error) { return (false); }i.mousemove((e) => { t.mlp = { x: e.pageX, y: e.pageY }; }), i.find('iframe').on('load', e); }t.mlp = { x: 0, y: 0 }, t(e), t.fn.ismouseover = function () {
        let e = !1; return this.eq(0).each(function () {
            let i = t(this).is('iframe') ? t(this).contents().find('body') : t(this),
                n = i.offset(); e = n.left <= t.mlp.x && n.left + i.outerWidth() > t.mlp.x && n.top <= t.mlp.y && n.top + i.outerHeight() > t.mlp.y;
        }), e;
    };
}(jQuery));
...
function UGThumbsGeneral() {
    ...
    let g_gallery = new UniteGalleryMain(),
        g_objGallery,
        g_objects,
        g_objWrapper;
    let g_arrItems,
        g_objStrip,
        g_objParent;
    const g_functions = new UGFunctions();
    let g_strip;
    let outer_options;
    ...
    const g_serviceParams = {			// service variables
    ...
    this.setHtmlThumbs = function (objParent) {
        ...
        const numItems = g_gallery.getNumItems();
        ...
        for (let i = 0; i < numItems; i++) {
            const objItem = g_arrItems[i];

            let classAddition = '';
            if (g_temp.customThumbs == false) { classAddition = ' ug-thumb-generated'; }

            const zIndex = objItem.index + 1;
            const thumbStyle = `style='z-index:${zIndex};'`;

            var htmlThumb = `<div class='ug-thumb-wrapper${classAddition}' ${thumbStyle}></div>`;

            if (g_options.thumb_wrapper_as_link == true) {
                let urlLink = objItem.link;
            ...
            const objThumbWrapper = jQuery(htmlThumb);

            const objImage = objItem.objThumbImage;

            if (g_temp.customThumbs == false) {
                if (g_options.thumb_show_loader == true && objImage) {
                    let loaderClass = 'ug-thumb-loader-dark';
                ...
                if (objImage) {
                    ...
                    if (g_options.thumb_image_overlay_effect == true) {
                        const objImageOverlay = objImage.clone().appendTo(objThumbWrapper);
    ...
    function setThumbSize(width, height, objThumb, innerOnly) {
        ...
        const selectorsString = '.ug-thumb-loader, .ug-thumb-error, .ug-thumb-border-overlay, .ug-thumb-overlay';
    ...
    function setThumbBorderEffect(objThumb, borderWidth, borderColor, noAnimation) {
        ...
        const objBorderOverlay = objThumb.children('.ug-thumb-border-overlay');
    ...
    function setThumbColorOverlayEffect(objThumb, isActive, noAnimation) {
        const objOverlay = objThumb.children('.ug-thumb-overlay');

        let animationDuration = g_options.thumb_transition_duration;
    ...
    function setThumbImageOverlayEffect(objThumb, isActive, noAnimation) {
        const objImage = objThumb.children('img.ug-thumb-image');
        const objImageOverlay = objThumb.children('img.ug-thumb-image-overlay');
    ...
    this.setThumbNormalStyle = function (objThumb, noAnimation, fromWhere) {
        ...
        if (g_temp.isEffectOverlay) {
            const isSet = g_options.thumb_overlay_reverse != true;
    ...
    function setItemThumbLoadedError(objThumb) {
        const objItem = t.getItemByThumb(objThumb);
    ...
    function placeThumbImage(objThumb, objImage, objItem) {
        ...
        if (g_options.thumb_fixed_size == true) {
            ...
            { g_functions.scaleImageByHeight(objImage, g_options.thumb_height); } else		// vertical strip
            { g_functions.scaleImageByWidth(objImage, g_options.thumb_width); }

            const imageSize = g_functions.getElementSize(objImage);
    ...
    function copyPositionToThumbOverlayImage(objImage) {
        const objImageBW = objImage.siblings('.ug-thumb-image-overlay');
        if (objImageBW.length == 0) { return (false); }

        const objSize = g_functions.getElementSize(objImage);
    ...
    function getImageEffectsClass() {
        ...
        const arrEffects = g_options.thumb_image_overlay_type.split(',');

        for (const index in arrEffects) {
            const effect = arrEffects[index];
    ...
    function onThumbSizeChange(temp, objThumb) {
        ...
        const objSize = g_functions.getElementSize(objThumb);
    ...
    function onImageLoaded(image, isForce) {
        ...
        const objImage = jQuery(image);
        const objThumb = objImage.parents('.ug-thumb-wrapper');
    ...
    function onThumbImageLoaded(data, objThumb, objImage) {
        ...
        const objSize = g_functions.getImageOriginalSize(objImage);
        ...
        if (g_temp.isEffectOverlay) {
            if (g_options.thumb_overlay_color) {
                ...
                if (g_functions.isRgbaSupported()) {
                    const colorRGB = g_functions.convertHexToRGB(g_options.thumb_overlay_color, g_options.thumb_overlay_opacity);
    ...
    this.getNumThumbs = function () {
        const numThumbs = g_arrItems.length;
        return (numThumbs);
    };
    ...
    this.getThumbImage = function (objThumb) {
        const objImage = objThumb.find('.ug-thumb-image');
        return (objImage);
    };
    ...
    this.getItemByThumb = function (objThumb) {
        let index = objThumb.data('index');

        if (index === undefined) { index = objThumb.index(); }

        const arrItem = g_arrItems[index];
        return (arrItem);
    };
    ...
    this.initEvents = function () {
        const objThumbs = g_objParent.find('.ug-thumb-wrapper');

        objThumbs.on('touchstart', () => {
    ...
    this.loadThumbsImages = function () {
        const objImages = g_objParent.find('.ug-thumb-image');
        g_functions.checkImagesLoaded(objImages, null, (objImage, isError) => {
            if (isError == false) {
                onImageLoaded(objImage, true);
            } else {
                const objItem = jQuery(objImage).parent();
...
function UGThumbsStrip() {
    ...
    let g_arrItems,
        ...
        g_objStripInner;
    var g_aviaControl,
        g_touchThumbsControl,
        g_functions = new UGFunctions();
    let g_isVertical = false,
    ...
    function onAfterSetOptions() {
        const thumbsOptions = g_thumbs.getOptions();
    ...
    function initSizeParams() {
        ...
        const arrThumbs = g_objStripInner.children('.ug-thumb-wrapper');
        const firstThumb = jQuery(arrThumbs[0]);
        const thumbsRealWidth = firstThumb.outerWidth();
        const thumbsRealHeight = firstThumb.outerHeight();
        const thumbs_options = g_thumbs.getOptions();
    ...
    function positionThumbs() {
        ...
        for (i = 0; i < arrThumbs.length; i++) {
            const objThumb = jQuery(arrThumbs[i]);
    ...
    function fixSize() {
        if (g_isVertical == false) {		// fix horizontal
            const height = g_sizes.thumbSecondSize;
            ...
            const width = g_sizes.thumbSecondSize;
    ...
    function scrollBy(scrollStep) {
        const innerPos = t.getInnerStripPos();
        let finalPos = innerPos + scrollStep;
    ...
    function scrollToThumbMin(objThumb) {
        const objThumbPos = getThumbPos(objThumb);

        let scrollPos = objThumbPos.min * -1;
    ...
    function scrollToThumbMax(objThumb) {
        ...
        let scrollPos = objThumbPos.max * -1 + g_sizes.stripSize;
        scrollPos = t.fixInnerStripLimits(scrollPos);
    ...
    function scrollToThumb(objThumb) {
        ...
        const objBounds = getThumbsInsideBounds();

        const objThumbPos = getThumbPos(objThumb);

        if (objThumbPos.min < objBounds.minPosThumbs) {
            const prevThumb = objThumb.prev();
            if (prevThumb.length) { scrollToThumbMin(prevThumb); } else { scrollToThumbMin(objThumb); }
        } else if (objThumbPos.max > objBounds.maxPosThumbs) {
            const nextThumb = objThumb.next();
    ...
    function scrollToSelectedThumb() {
        const selectedItem = g_gallery.getSelectedItem();
        if (selectedItem == null) { return (true); }

        const objThumb = selectedItem.objThumbWrapper;
    ...
    function checkAndRepositionInnerStrip() {
        ...
        const pos = t.getInnerStripPos();

        const posFixed = t.fixInnerStripLimits(pos);
    ...
    function checkControlsEnableDisable() {
        const isMovingEndbled = isStripMovingEnabled();
    ...
    function onThumbClick(objThumb) {
        ...
        if (t.isTouchMotionActive()) {
            const isSignificantPassed = g_touchThumbsControl.isSignificantPassed();
            if (isSignificantPassed == true) { return (true); }
        }

        // run select item operation
        const objItem = g_thumbs.getItemByThumb(objThumb);
    ...
    function onItemChange() {
        const objItem = g_gallery.getSelectedItem();
    ...
    function initEvents() {
        ...
        const objThumbs = g_objStrip.find('.ug-thumb-wrapper');

        objThumbs.on('click touchend', function (event) {
            const clickedThumb = jQuery(this);
    ...
    function isStripMovingEnabled() {
        if (g_sizes.stripInnerSize > g_sizes.stripActiveSize) { return (true); }
        return (false);
    }
    ...
    function getThumbPos(objThumb) {
        ...
        const objPos = objThumb.position();
    ...
    this.positionInnerStrip = function (pos, isAnimate) {
        ...
        if (g_isVertical == false) { var objPosition = { left: `${pos}px` }; } else { var objPosition = { top: `${pos}px` }; }
        ...
        if (isAnimate == false) {		// normal position
            ...
            g_objStripInner.stop(true).animate(objPosition, {
                ...
                queue: false,
                progress() { t.triggerStripMoveEvent(); },
                always() { t.triggerStripMoveEvent(); },
            });
        }
    };
    ...
    this.isTouchMotionActive = function () {
        ...
        const isActive = g_touchThumbsControl.isTouchActive();
    ...
    this.isItemThumbVisible = function (objItem) {
        const objThumb = objItem.objThumbWrapper;
        const thumbPos = objThumb.position();

        const posMin = t.getInnerStripPos() * -1;
    ...
    this.getInnerStripPos = function () {
        if (g_isVertical == false) { return g_objStripInner.position().left; }
    ...
    this.fixInnerStripLimits = function (distPos) {
        ...
        const objLimits = t.getInnerStripLimits();
    ...
    this.setSizeHorizontal = function (width) {
        ...
        const height = g_sizes.thumbSecondSize + g_options.strip_padding_top + g_options.strip_padding_bottom;
        ...
        const innerLeft = g_options.strip_padding_left;
    ...
    this.getObjects = function () {
        const thumbsOptions = g_thumbs.getOptions();
        const commonOpitions = jQuery.extend(g_options, thumbsOptions);
    ...
    this.getSelectedThumb = function () {
        const selectedIndex = g_gallery.getSelectedItemIndex();
    ...
    this.getSizeAndPosition = function () {
        const obj = g_functions.getElementSize(g_objStrip);
    ...
    this.getHeight = function () {
        const stripHeight = g_objStrip.outerHeight();
    ...
    this.getWidth = function () {
        const stripWidth = g_objStrip.outerWidth();
    ...
    this.isMoveEnabled = function () {
        const isEnabled = isStripMovingEnabled();
        return (isEnabled);
    };
}
...
function UGTouchThumbsControl() {
    ...
    function getMouseOrientPosition(event) {
        if (g_isVertical == false) { return (g_functions.getMousePosition(event).pageX); }
    ...
    function handleStripDrag(mousePos) {
        const diff = g_temp.mousePos - mousePos;
        let distPos = g_temp.innerPos - diff;

        // make harder to drag the limits
        const objLimits = g_parent.getInnerStripLimits();
    ...
    function storeInitTouchValues(pagePos) {
        const currentInnerPos = g_parent.getInnerStripPos();
    ...
    function storeTouchPortionData() {
        // calc speed
        const currentTime = jQuery.now();
        const deltaTime = currentTime - g_temp.lastTime;
    ...
    function continueTouchMotion() {
        const slowFactor = g_serviceParams.thumb_touch_slowFactor;

        // don't alow portion less then the minTime
        const minDeltaTime = g_serviceParams.minDeltaTime;

        // if less then this path, dont' continue motion
        const minPath = g_serviceParams.minPath;

        const currentInnerPos = g_parent.getInnerStripPos();

        const currentTime = jQuery.now();
        let deltaTime = currentTime - g_temp.lastTime;
        let deltaPos = currentInnerPos - g_temp.lastPortionPos;
        ...
        const dir = (deltaPos > 0) ? 1 : -1;

        let speed = 0;
        if (deltaTime > 0) { speed = deltaPos / deltaTime; }

        let path = (speed * speed) / (slowFactor * 2) * dir;

        // disable path for very slow motions
        if (Math.abs(path) <= minPath) { path = 0; }


        const innerStripPos = g_parent.getInnerStripPos();
        const newPos = innerStripPos + path;

        let correctPos = g_parent.fixInnerStripLimits(newPos);
        const objLimits = g_parent.getInnerStripLimits();

        // check the off the limits and return (second animation)
        const limitsBreakAddition = g_serviceParams.limitsBreakAddition;
        let doQueue = false;
        const returnPos = correctPos;
        ...
        if (newPos < objLimits.minPos) {
            doQueue = true;
            const maxStopPos = objLimits.minPos - limitsBreakAddition;
            correctPos = maxStopPos;
            if (newPos > maxStopPos) { correctPos = newPos; }
        }

        const correctPath = correctPos - innerStripPos;

        // set animation speed
        let animateSpeed = Math.abs(Math.round(speed / slowFactor));
        ...
        if (innerStripPos != correctPos) {
            let animateProps = { left: `${correctPos}px` };
            if (g_isVertical == true) { animateProps = { top: `${correctPos}px` }; }
        ...
        if (doQueue == true) {
            const returnAnimateSpeed = g_serviceParams.returnAnimateSpeed;

            let returnAnimateProps = { left: `${returnPos}px` };
            if (g_isVertical == true) { returnAnimateProps = { top: `${returnPos}px` }; }
    ...
    function endStripTouchLoop(event) {
        ...
        if (event) {
            const pagePos = getMouseOrientPosition(event);
...
function UGPanelsBase() {
    ...
    this.setHtml = function (g_objPanelArg) {
        ...
        if (g_temp.isDisabledAtStart === true) {
            const html = "<div class='ug-overlay-disabled'></div>";
    ...
    this.initEvents = function () {
        ...
        if (g_objHandle) {
            ...
            g_objGallery.on(g_gallery.events.SLIDER_ACTION_START, () => {
                g_objHandle.hideHandle();
            });

            g_objGallery.on(g_gallery.events.SLIDER_ACTION_END, () => {
    ...
    this.closePanel = function (noAnimation) {
        ...
        const panelDest = t.getClosedPanelDest();
    ...
    this.getClosedPanelDest = function () {
        let objPanelSize = g_functions.getElementSize(g_objPanel),
            panelDest;
...
function UGPanelHandle() {
    ...
    this.init = function (panel, objPanel, panelOptions, panelType, gallery) {
        ...
        // set arrows skin:
        const galleryOptions = gallery.getOptions();
        const globalSkin = galleryOptions.gallery_skin;
    ...
    this.setHtml = function () {
        const orientation = g_panel.getOrientation();

        let classTip = 'ug-panel-handle-tip';
    ...
    this.initEvents = function () {
        ...
        // on panel open
        jQuery(g_panel).on(g_panel.events.OPEN_PANEL, () => {
            removeHoverState();
            removeClosedState();
        });

        // one panel close
        jQuery(g_panel).on(g_panel.events.CLOSE_PANEL, () => {
            removeHoverState();
            setClosedState();
        });
    };
    ...
    this.placeHandle = function () {
        const handleSize = g_functions.getElementSize(g_objHandleTip);
...
function UGStripPanel() {
    ...
    let g_gallery = new UniteGalleryMain(),
        ...
        g_objWrapper,
        g_objPanel;
    let g_functions = new UGFunctions(),
    ...
    function initPanel(gallery, customOptions) {
        ...
        if (repeatCustomOptions == true) { g_options = jQuery.extend(g_options, customOptions); } // do the


        // set arrows skin:
        const galleryOptions = g_gallery.getOptions();
        const globalSkin = galleryOptions.gallery_skin;
    ...
    function setElementsSize_hor() {
        // get strip height
        const stripHeight = g_objStrip.getHeight();
        const panelWidth = g_temp.panelWidth;
        ...
        if (g_objButtonNext) {
            ...
            // arrange buttons tip
            const prevTip = g_objButtonPrev.children('.ug-strip-arrow-tip');
            g_functions.placeElement(prevTip, 'center', 'middle');

            const nextTip = g_objButtonNext.children('.ug-strip-arrow-tip');
            g_functions.placeElement(nextTip, 'center', 'middle');
        }

        // set panel height
        const panelHeight = stripHeight + g_options.strippanel_padding_top
        ...
        // set strip size
        let stripWidth = panelWidth - g_options.strippanel_padding_left	- g_options.strippanel_padding_right;

        if (g_objButtonNext) {
            const buttonWidth = g_objButtonNext.outerWidth();
    ...
    function setElementsSize_vert() {
        // get strip width
        const stripWidth = g_objStrip.getWidth();
        const panelHeight = g_temp.panelHeight;
        ...
        // set panel width
        const panelWidth = stripWidth + g_options.strippanel_padding_left
        ...
        // set strip size
        let stripHeight = panelHeight - g_options.strippanel_padding_top
            - g_options.strippanel_padding_bottom;

        if (g_objButtonNext) {
            const buttonHeight = g_objButtonNext.outerHeight();
    ...
    function placeElements_hor() {
        ...
        let stripX = g_options.strippanel_padding_left;
        if (g_objButtonNext) {
    ...
    function placeElements_vert() {
        ...
        let stripY = g_options.strippanel_padding_top;
        if (g_objButtonNext) {
    ...
    function checkSideButtons() {
        ...
        // check the limits
        const limits = g_objStrip.getInnerStripLimits();
        const pos = g_objStrip.getInnerStripPos();
    ...
    this.getSize = function () {
        const objSize = g_functions.getElementSize(g_objPanel);
...
function UGGridPanel() {
    ...
    let g_gallery = new UniteGalleryMain(),
        ...
        g_objPanel;
    const g_functions = new UGFunctions();
    let g_objGrid = new UGThumbsGrid();
    const g_panelBase = new UGPanelsBase();
    let g_objArrowNext,
        g_objArrowPrev;
    ...
    function initGridPanel(gallery, customOptions) {
        ...
        const objects = gallery.getObjects();
    ...
    function getGridMaxHeight() {
        ...
        let maxGridHeight = g_temp.panelHeight - spaceTaken;

        if (g_options.gridpanel_arrows_always_on == false) {
            const numPanes = g_objGrid.getNumPanesEstimationByHeight(maxGridHeight);
            if (numPanes == 1) { return (maxGridHeight); }
        }

        // count the size with arrows
        const arrowsSize = g_functions.getElementSize(g_objArrowNext);
        const arrowsHeight = arrowsSize.height;
    ...
    function getGridMaxWidth() {
        // check space taken without arrows for one pane grids
        let spaceTaken = g_options.gridpanel_padding_border_left + g_options.gridpanel_padding_border_right;

        let maxGridWidth = g_temp.panelWidth - spaceTaken;

        if (g_options.gridpanel_arrows_always_on == false) {
            const numPanes = g_objGrid.getNumPanesEstimationByWidth(maxGridWidth);
        ...
        // count the size with arrows
        const arrowsSize = g_functions.getElementSize(g_objArrowNext);
        const arrowsWidth = arrowsSize.width;
    ...
    function setArrows() {
        ...
        if (g_options.gridpanel_arrows_always_on == true) {
            ...
            const numPanes = g_objGrid.getNumPanes();
    ...
    function setPanelSize() {
        const gridSize = g_objGrid.getSize();
    ...
    function getGridPaddingLeft() {
        const gridPanelLeft = g_options.gridpanel_padding_border_left;
    ...
    function placeElements_noarrows() {
        // place grid
        let gridY = g_options.gridpanel_grid_align,
            gridPaddingY = 0;
        ...
        const gridPanelLeft = getGridPaddingLeft();

        const gridElement = g_objGrid.getElement();
    ...
    function placeElementsVert_arrows() {
        ...
        let gridY,
            ...
            nextArrowY,
            nextArrowPaddingY;
        const objArrowSize = g_functions.getElementSize(g_objArrowPrev);
    ...
    function placeElementsHor_arrows() {
        ...
        // place grid
        let gridY = g_options.gridpanel_padding_border_top;
    ...
    function placeElementsHorType_arrows() {
        ...
        let gridX,
            ...
            arrowsY;
        const objArrowSize = g_functions.getElementSize(g_objArrowPrev);
        let objGridSize = g_objGrid.getSize();
    ...
    function placeElementHorType_noarrows() {
        let gridX;
        const objGridSize = g_objGrid.getSize();
    ...
    this.setHeight = function (height) {
        ...
        g_temp.panelHeight = height;
        const gridMaxHeight = getGridMaxHeight();
    ...
    this.setWidth = function (width) {
        ...
        const gridMaxWidth = getGridMaxWidth();
...
function UGThumbsGrid() {
    ...
    let g_functions = new UGFunctions(),
        ...
        g_objGrid,
        g_objInner;
    let g_thumbs = new UGThumbsGeneral(),
    ...
    function init(gallery, customOptions, isTilesMode) {
        ...
        if (g_temp.isTilesMode == true) {
            ...
            const options_td = g_tilesDesign.getOptions();
    ...
    function validateBeforeRun() {
        ...
        } else if (g_temp.gridWidth == 0) { throw new Error('You must set width before run.'); }
    }
    ...
    function run() {
        ...
        } else if (g_temp.isTilesMode == true) {
            // check if dynamic size changed. If do, run the thumbs again
            const isChanged = initGridDynamicSize();
        ...
        if (g_temp.isFirstTimeRun == true && g_temp.isTilesMode) {
            const objTiles = g_thumbs.getThumbs();

            // fire size change event
            objTiles.each((index, tile) => {
    ...
    function initGridDynamicSize() {
        ...
        let isChanged = false;
        const isMobile = g_gallery.isMobileMode();

        // --- set space between cols and rows

        const spaceOld = g_temp.spaceBetweenCols;
        ...
        const lastThumbSize = getThumbsSize();
        const lastThumbWidth = lastThumbSize.width;

        let tileWidth = g_temp.tileMaxWidth;
        const numCols = g_functions.getNumItemsInSpace(g_temp.gridWidth, g_temp.tileMaxWidth, g_temp.spaceBetweenCols);
    ...
    function initSizeParamsHor() {
        const objThumbSize = getThumbsSize();

        const thumbsHeight = objThumbSize.height;

        // set grid size
        const gridWidth = g_temp.gridWidth;
        const gridHeight = g_options.grid_num_rows * thumbsHeight + (g_options.grid_num_rows - 1) * g_temp.spaceBetweenRows + g_options.grid_padding * 2;
    ...
    function initSizeParamsVert() {
        ...
        const thumbsWidth = objThumbSize.width;

        // set grid size
        const gridWidth = g_options.grid_num_cols * thumbsWidth + (g_options.grid_num_cols - 1) * g_temp.spaceBetweenCols + g_options.grid_padding * 2;
        const gridHeight = g_temp.gridHeight;
    ...
    function scrollToThumb(thumbIndex) {
        const paneIndex = getPaneIndexByThumbIndex(thumbIndex);
    ...
    function positionThumb_hor() {
        const arrThumbs = g_objInner.children('.ug-thumb-wrapper');
        ...
        const numThumbs = arrThumbs.length;

        for (i = 0; i < numThumbs; i++) {
            const objThumb = jQuery(arrThumbs[i]);
            g_functions.placeElement(objThumb, posx, posy);

            const thumbWidth = objThumb.outerWidth();
            const thumbHeight = objThumb.outerHeight();

            // count maxx
            if (posx > maxx) { maxx = posx; }

            // count maxy
            const endY = posy + thumbHeight;
            if (endY > maxy) { maxy = endY; }

            // count maxx end
            const endX = maxx + thumbWidth;
    ...
    function positionThumb_vert() {
        ...
        for (i = 0; i < numThumbs; i++) {
            ...
            let paneMaxY = paneStartY + g_temp.gridHeight;
    ...
    function positionThumb_hortype() {
        ...
        let baseX = g_options.grid_padding;
        const baseY = g_options.grid_padding;
        let posy = baseY;
        let posx = baseX;
        let maxx = 0,
            maxy = 0,
    ...
    function setInnerPos(pos) {
        const objCss = getInnerPosObj(pos);
    ...
    function animateToCurrentPane() {
        const innerPos = -g_temp.arrPanes[g_temp.currentPane];
    ...
    function getInnerSize() {
        if (g_temp.isNavigationVertical == true) { return (g_temp.innerHeight); }
    ...
    function getPaneSize() {
        if (g_temp.isNavigationVertical == true) { return (g_temp.gridHeight); }
    ...
    function getMousePos(event) {
        const mousePos = g_functions.getMousePosition(event);

        if (g_temp.isNavigationVertical == true) { return (mousePos.pageY); }
    ...
    function getInnerPos() {
        const objSize = g_functions.getElementSize(g_objInner);

        if (g_temp.isNavigationVertical == true) { return (objSize.top); }
        return (objSize.left);
    }
    ...
    function getPaneIndexByThumbIndex(thumbIndex) {
        ...
        const numPane = Math.floor(thumbIndex / g_temp.numThumbsInPane);
    ...
    function getPanePosition(index) {
        const pos = g_temp.arrPanes[index];
        return (pos);
    }
    ...
    function isSignificantPassed() {
        ...
        const objData = g_functions.getStoredEventData(g_temp.storedEventID);

        const passedTime = objData.diffTime;

        const currentInnerPos = getInnerPos();
        const passedDistanceAbs = Math.abs(currentInnerPos - objData.startInnerPos);
    ...
    function isMovementValidForChange() {
        ...
        const breakSize = Math.round(paneSize * 3 / 8);
    ...
    function isApproveTileClick() {
        ...
        const isApprove = g_functions.isApproveStoredEventClick(g_temp.storedEventID, g_temp.isNavigationVertical);
    ...
    function handleScrollTop() {
        ...
        const scrollDir = g_functions.handleScrollTop(g_temp.storedEventID);
    ...
    function onTouchMove(event) {
        ...
        const objData = g_functions.getStoredEventData(g_temp.storedEventID, g_temp.isNavigationVertical);

        // check if was vertical scroll
        const isScroll = handleScrollTop();
        if (isScroll) { return (true); }


        const diff = objData.diffMousePos;
        let innerPos = objData.startInnerPos + diff;
        const direction = (diff > 0) ? 'prev' : 'next';
        const lastPaneSize = g_temp.arrPanes[g_temp.numPanes - 1];
    ...
    function onTouchEnd(event) {
        ...
        const innerPos = getInnerPos();
        const diff = innerPos - objData.startInnerPos;
        const direction = (diff > 0) ? 'prev' : 'next';
    ...
    function initEvents() {
        if (g_temp.isTilesMode == false) {
            g_thumbs.initEvents();
            const objThumbs = g_objGrid.find('.ug-thumb-wrapper');
     ...
     */
    this.isItemThumbVisible = function (objItem) {
        const itemIndex = objItem.index;
        const paneIndex = getPaneIndexByThumbIndex(itemIndex);
    ...
    this.getNumPanesEstimationByHeight = function (gridHeight) {
        ...
        const numThumbs = g_thumbs.getNumThumbs();
        const numRows = Math.ceil(numThumbs / g_options.grid_num_cols);

        const totalHeight = numRows * thumbHeight + (numRows - 1) * g_temp.spaceBetweenRows;

        const numPanes = Math.ceil(totalHeight / gridHeight);
    ...
    this.getNumPanesEstimationByWidth = function (gridWidth) {
        ...
        const numThumbs = g_thumbs.getNumThumbs();
        const numCols = Math.ceil(numThumbs / g_options.grid_num_rows);

        const totalWidth = numCols * thumbWidth + (numCols - 1) * g_temp.spaceBetweenCols;

        const numPanes = Math.ceil(totalWidth / gridWidth);
    ...
    this.getHeightEstimationByWidth = function (width) {
        ...
        const numThumbs = g_thumbs.getNumThumbs();
        const numCols = g_functions.getNumItemsInSpace(width, g_options.tile_width, g_temp.spaceBetweenCols);
        let numRows = Math.ceil(numThumbs / numCols);

        if (numRows > g_options.grid_num_rows) { numRows = g_options.grid_num_rows; }

        let gridHeight = g_functions.getSpaceByNumItems(numRows, g_options.tile_height, g_temp.spaceBetweenRows);
     ...
     */
    this.getSize = function () {
        const objSize = g_functions.getElementSize(g_objGrid);
        return (objSize);
    };
    ...
    this.gotoPane = function (index, fromWhere) {
        ...
        const innerPos = -g_temp.arrPanes[index];
    ...
    this.nextPane = function () {
        let nextPaneIndex = g_temp.currentPane + 1;
    ...
    this.prevPane = function () {
        let prevPaneIndex = g_temp.currentPane - 1;
        if (prevPaneIndex < 0) {
    ...
    this.attachNextPaneButton = function (objButton) {
        ...
        g_objThis.on(t.events.PANE_CHANGE, () => {
    ...
    this.attachBullets = function (objBullets) {
        ...
        jQuery(objBullets).on(objBullets.events.BULLET_CLICK, (data, numBullet) => {
            t.gotoPane(numBullet, 'theme');
            objBullets.setActive(numBullet);
        });

        jQuery(t).on(t.events.PANE_CHANGE, (data, numPane) => {
...
function UGTiles() {
    ...
    function setHtml(objParent) {
        ...
        const tilesType = g_options.tiles_type;
    ...
    function setTransition() {
        ...
        if (g_options.tiles_enable_transition == true) {
            ...
            const optionsTile = g_objTileDesign.getOptions();
    ...
    function fillTilesVars() {
        ...
        if (g_options.tiles_exact_width == true) {
            ...
            let numCols = g_functions.getNumItemsInSpaceRound(g_vars.availWidth, g_vars.colWidth, g_vars.colGap);

            if (numCols < g_vars.minCols) { numCols = g_vars.minCols; } else
            if (g_vars.maxCols != 0 && numCols > g_vars.maxCols) { numCols = g_vars.maxCols; }
        ...
        for (col = 0; col < g_vars.numCols; col++) {
            const colX = g_functions.getColX(col, g_vars.colWidth, g_vars.colGap);
    ...
    function placeTile(objTile, toShow, setGalleryHeight, numCol) {
        ...
        let itemHeight = g_objTileDesign.getTileHeightByWidth(g_vars.colWidth, objTile, 'placeTile');

        if (itemHeight === null) {	// for custom html tile
            if (g_options.tiles_enable_transition == true) { throw new Error("Can't know tile height, please turn off transition"); }

            const itemSize = g_functions.getElementSize(objTile);
            itemHeight = itemSize.height;
        }

        const posx = g_vars.arrPosx[numCol];

        g_functions.placeElement(objTile, posx, posy);

        const realHeight = posy + itemHeight;
    ...
    function placeTiles(toShow) {
        ...
        const objThumbs = g_thumbs.getThumbs();
        ...
        // place elements
        for (let index = 0; index < objThumbs.length; index++) {
            const objTile = jQuery(objThumbs[index]);
            let col;
        ...
        const galleryHeight = g_objParent.height();

        if (g_temp.isTransActive == true && galleryHeight > g_vars.maxColHeight) {
            setTimeout(() => {
                g_objParent.height(g_vars.maxColHeight);
            }, 700);
        } else { g_objParent.height(g_vars.maxColHeight); }
    }
    ...
    function isOrderedTilePlaceAlowed(objTile) {
        const index = objTile.index();

        // don't allow double put items
        const currentItem = g_gallery.getItem(index);
        if (currentItem.ordered_placed === true) { return (false); }


        const prevIndex = g_functions.getPrevRowSameColIndex(index, g_vars.numCols);

        // put first item in the column
        if (prevIndex < 0) { return (true); }

        // check if previous tile in column is placed
        const objPrevItem = g_gallery.getItem(prevIndex);
    ...
    function placeOrderedTile(objTile, isForce) {
        if (isForce !== true) {
            const isAlowed = isOrderedTilePlaceAlowed(objTile);
        ...
        const col = g_functions.getColByIndex(g_vars.numCols, index);

        const objItem = g_gallery.getItem(index);
        ...
        const numItems = g_gallery.getNumItems();
        const nextIndex = g_functions.getNextRowSameColIndex(index, g_vars.numCols);
        if (nextIndex >= numItems) { return (false); }

        const nextTile = g_thumbs.getThumbByIndex(nextIndex);
        const nextItem = g_gallery.getItem(nextIndex);

        const isLoaded = g_thumbs.isThumbLoaded(nextTile);
    ...
    function onSingleImageLoad(objImage, isError) {
        ...
        const objTile = jQuery(objImage).parent();
    ...
    function runColumnsType() {
        ...
        const diffWidth = Math.abs(g_vars.galleryWidth - g_vars.totalWidth);

        // set initial height of the parent by estimation
        if (g_options.tiles_set_initial_height == true && g_functions.isScrollbarExists() == false && diffWidth < 25) {
            const numThumbs = objThumbs.length;
            const numRows = Math.ceil(objThumbs.length / g_vars.numCols);
            const estimateHeight = numRows * g_options.tiles_col_width * 0.75;
        ...
        const objImages = jQuery(g_objParent).find('img.ug-thumb-image');
        ...
        if (g_temp.isFixedMode == true) {		// fixed mode type - just place tiles before images loaded
            ...
            g_functions.checkImagesLoaded(objImages, () => {
                setTransition();
                g_objThis.trigger(t.events.ALL_TILES_LOADED);
            });
        } else {	// dynamic size type
            const initNumCols = g_vars.numCols;
            const initWidth = g_vars.galleryWidth;
    ...
    function getJustifiedData() {
        const galleryWidth = getParentWidth();

        const objTiles = g_thumbs.getThumbs();
        const rowHeightOpt = g_options.tiles_justified_row_height;
        const arrWidths = [];
        let totalWidth = 0;
        const gap = g_options.tiles_justified_space_between;
        const numTiles = objTiles.length;

        // get arr widths and total width
        jQuery.each(objTiles, (index, objTile) => {
            objTile = jQuery(objTile);

            const objItem = g_thumbs.getItemByThumb(objTile);

            let tileWidth = objItem.thumbWidth;
            const tileHeight = objItem.thumbHeight;
        ...
        let numRows = Math.ceil(totalWidth / galleryWidth);

        if (numRows > numTiles) { numRows = numTiles; }

        const finalRowWidth = totalWidth / numRows;
        ...
        jQuery.each(objTiles, (index, objTile) => {
            const tileWidth = arrWidths[index];
        ...
        jQuery.each(arrRows, (index, row) => {
            const numTiles = row.length;
            const rowWidth = rowsWidths[index];

            const gapWidth = (row.length - 1) * gap;

            const ratio = (galleryWidth - gapWidth) / rowWidth;
            const rowHeight = Math.round(rowHeightOpt * ratio);
            ...
            const ratioHeights = rowHeight / rowHeightOpt;

            // set tiles sizes:
            const arrRowTileWidths = [];
            let actualRowWidth = gapWidth;

            jQuery.each(row, (indexInRow, tile) => {
                const objTile = jQuery(tile);
                const tileIndex = objTile.index();
                const tileWidth = arrWidths[tileIndex];
                const newWidth = Math.round(tileWidth * ratioHeights);
            ...
            let diff = actualRowWidth - galleryWidth;

            const newTotal = 0;
            jQuery.each(arrRowTileWidths, (indexInRow, width) => {
    ...
    function placeJustified(toShow) {
        ...
        let objData = getJustifiedData();

        // if the width changed after height change (because of scrollbar), recalculate
        g_objParent.height(objData.totalHeight);

        const parentWidthAfter = getParentWidth();
        if (parentWidthAfter != parentWidth) { objData = getJustifiedData(); }

        doBeforeTransition();

        let posy = 0;
        let totalWidth = 0;		// just count total widht for check / print
        jQuery.each(objData.arrRows, (index, row) => {
            const arrRowTileWidths = objData.arrRowWidths[index];
            const rowHeight = objData.arrRowHeights[index];
            ...
            jQuery.each(row, (indexInRow, tile) => {
                ...
                const tileWidth = arrRowTileWidths[indexInRow];
    ...
    function runJustifiedType() {
        const objImages = jQuery(g_objWrapper).find('img.ug-thumb-image');
        const objTiles = g_thumbs.getThumbs();
    ...
    function fillNestedVars() {
        ...
        objTiles.each(function () {
            const objTile = jQuery(this);
            const sizes = setNestedSize(objTile);
    ...
    function placeNestedImages(toShow) {
        ...
        const totalHeight = g_nestedWork.maxGridY * (g_nestedWork.colWidth + g_nestedWork.currentGap) - g_nestedWork.currentGap;
    ...
    function setNestedSize(objTile) {
        ...
        const colWidth = g_nestedWork.colWidth;
        const gapWidth = g_nestedWork.currentGap;

        const objImageSize = g_objTileDesign.getTileImageSize(objTile);
        const index = objTile.index();

        const maxDim = Math.ceil(getPresettedRandomByWidth(index) * (g_nestedWork.nestedOptimalCols * 1 / 3) + g_nestedWork.nestedOptimalCols * 2 / 3);

        const imgWidth = objImageSize.width;
        const imgHeight = objImageSize.height;
    ...
    function placeNestedImagesCycle(placeOne) {
        ...
        for (let i = 0; i < g_arrNestedItems.length; i++) { placeNestedImage(i, true); }
    }
    ...
    function placeNestedImage(tileID, toShow) {
        ...
        const rowsValue = g_functions.getObjectLength(g_arrNestedGridRow);

        for (let row = g_nestedWork.gridY; row <= rowsValue + 1; row++) {
            for (let column = 0; column < g_nestedWork.maxColumns; column++) {
                if (isGridRowExists(g_nestedWork.gridY) == false || isGridCellTaken(g_nestedWork.gridY, column) == false) {
                    const totalFree = getTotalFreeFromPosition(column);
    ...
    function calculateOptimalStretchAndPosition(tileID, totalFree, column) {
        let sizes = jQuery.extend(true, {}, g_arrNestedItems[tileID]);
        const currentWidth = sizes.dimWidth;
        const placeForNextImage = totalFree - sizes.dimWidth;
        const optimalWidth = g_nestedWork.nestedOptimalCols;
        const onlyCurrentImage = (totalFree <= sizes.dimWidth || placeForNextImage <= 0.33 * optimalWidth || totalFree <= optimalWidth);
        ...
        } else if (isGridImageAligned(currentWidth, column) == true) {
        ...
        const columnInfo = getGridColumnHeight(tileID, sizes.dimWidth, column); 	// [columnHeight, imagesIDs]

        if (g_nestedWork.columnsValueToEnableHeightResize <= columnInfo[0] && g_nestedWork.maxColumns >= 2 * g_nestedWork.nestedOptimalCols) {
            const sideHelper = getGridImageVerticalDifference(column, sizes);
            const columnSizes = resizeToNewHeight(tileID, sideHelper.newHeight, true);
            g_arrNestedItems[tileID].dimHeight = columnSizes.dimHeight;
            const columnResizes = redistributeColumnItems(columnInfo, columnSizes.dimWidth, column);
            const columnCrosshairs = getColumnCrosshairsCount(columnResizes);
            let disableColumnResizes = false;
            ...
            const sideResizes = getSideResizeInfo(sideHelper.idToResize, sideHelper.newHeight, sizes.dimHeight);
            sizes.top = g_nestedWork.gridY;
            sizes.left = column;
            sideResizes.push({ tileID, sizes });

            const sideResizesVal = calcResizeRatio(sideResizes);
            const columnResizesVal = calcResizeRatio(columnResizes);
    ...
    function getColumnCrosshairsCount(tilesAndSizes) {
        let crosshairsCountR = 0;
        let crosshairsCountL = 0;
    ...
    function getSideResizeInfo(idToResize, newHeight, dimHeight) {
        const currentTile = g_arrNestedItems[idToResize];
        const	tileHeight = currentTile.dimHeight;
        const tileWidth = currentTile.dimWidth;
        const tileLeft = currentTile.left;
        const tileTop = currentTile.top;
        const tileDimTop = parseInt(tileTop / (g_nestedWork.colWidth + g_nestedWork.currentGap));
        const tileDimLeft = parseInt(tileLeft / (g_nestedWork.colWidth + g_nestedWork.currentGap));
        const newSideHeight = tileHeight - newHeight + dimHeight;

        const sideSizes = resizeToNewHeight(idToResize, newSideHeight, true);
        const output = [];
    ...
    function applyResizes(resizeTilesAndSizes) {
        for (let i = 0; i < resizeTilesAndSizes.length; i++) {
            const sizes = resizeTilesAndSizes[i].sizes;
            const tileID = resizeTilesAndSizes[i].tileID;
    ...
    function getGridColumnHeight(tileID, dimWidth, column) {
        let tempY = g_nestedWork.gridY - 1;
        let curImage = 0;
        let prevImage = 0;
        let columnHeight = 1;
        const imagesIDs = [];
    ...
    function getGridImageVerticalDifference(column, sizes) {
        let newHeightR = 0;
        let newHeightL = 0;
        const dimWidth = sizes.dimWidth;
        const dimHeight = sizes.dimHeight;
        let leftID = 0;
        let rightID = 0;
        const array = jQuery.map(g_arrNestedGridRow, (value, index) => [value]);
        ...
        const output = { newHeight, idToResize };
    ...
    function resizeToNewWidth(tileID, newDimWidth, toReturn) {
        ...
        const colWidth = g_nestedWork.colWidth;
        const gapWidth = g_nestedWork.currentGap;
        const currentTile = g_arrNestedItems[tileID];
        const imgWidth = currentTile.imgWidth;
        const imgHeight = currentTile.imgHeight;
        const ratio = imgWidth / imgHeight;
        dimWidth = newDimWidth;
        dimHeight = Math.round(dimWidth / ratio);
        if (toReturn == true) {
            const sizes = jQuery.extend(true, {}, currentTile);
    ...
    function resizeToNewHeight(tileID, newDimHeight, toReturn) {
        ...
        const currentTile = g_arrNestedItems[tileID];
        const dimWidth = currentTile.dimWidth;
        const colWidth = g_nestedWork.colWidth;
        const gapWidth = g_nestedWork.currentGap;
    ...
    function calcResizeRatio(objTilesAndSizes) {
        ...
        for (let i = 0; i < objTilesAndSizes.length; i++) {
            const sizes = g_arrNestedItems[objTilesAndSizes[i].tileID];
    ...
    function putImageToGridRow(id, sizes, gridX, gridY) {
        for (let y = 0; y < sizes.dimHeight; y++) {
            for (let x = 0; x < sizes.dimWidth; x++) {
    ...
    function addGridRow(y) {
        ...
        for (let t = 0; t < g_nestedWork.maxColumns; t++) {
    ...
    function drawNestedImages(toShow) {
        ...
        for (let i = 0; i < g_arrNestedItems.length; i++) {
    ...
    function drawNestedImage(i, toShow) {
        const objTile = g_thumbs.getThumbByIndex(i);

        const sizes = g_arrNestedItems[i];
        const newY = sizes.top * (g_nestedWork.colWidth + g_nestedWork.currentGap);
        const newX = g_nestedWork.addX + sizes.left * (g_nestedWork.colWidth + g_nestedWork.currentGap);
    ...
    function initEvents() {
        ...
        g_objThis.on(t.events.ALL_TILES_LOADED, () => {
...
function UGTileDesign() {
    ...
    function init(gallery, customOptions) {
        ...
        const objects = g_gallery.getObjects();
        g_objWrapper = objects.g_objWrapper;
        ...
        const objCustomOptions = { allow_onresize: false };

        let customThumbsAdd = ['overlay'];
        ...
        const thumbOptions = g_thumbs.getOptions();
    ...
    function modifyOptionsBeforeRender() {
        ...
        switch (g_options.tile_videoplay_icon_always_on) {
            ...
            case 'never':
                ...
                break;
            case 'mobile_only':
                g_temp.isVideoplayIconAlwaysOn = (isMobile == true);
                break;
            case 'desktop_only':
                g_temp.isVideoplayIconAlwaysOn = (isMobile == false);
                break;
        }
    }
    ...
    function setHtmlThumb(objThumbWrapper, objItem) {
        ...
        if (g_options.tile_enable_shadow == true) {
            let htmlShadow = `${g_options.tile_shadow_h}px `;
        ...
        if (g_options.tile_enable_icons) {
            ...
            let toSaparateIcon = g_temp.isSaparateIcons;
            if (toSaparateIcon == false && objItem.type != 'image' && g_temp.isVideoplayIconAlwaysOn == true) { toSaparateIcon = true; }

            if (toSaparateIcon)		// put the icons on the thumb
            { var objOverlay = objThumbWrapper; } else { var objOverlay = objThumbWrapper.children('.ug-thumb-overlay'); }

            objOverlay.append(htmlAdd);

            let objButtonZoom = objOverlay.children(`.${iconPlayClass}`);

            if (objButtonZoom.length == 0) { objButtonZoom = null; } else { objButtonZoom.hide(); }

            let objButtonLink = objOverlay.children('.ug-icon-link');
        ...
        if (g_options.tile_enable_image_effect == true) {
            let imageEffectClassAdd = '';
            if (g_options.tile_image_effect_reverse == false) { imageEffectClassAdd = ' ug-trans-enabled'; }

            let imageOverlayHtml = `<div class='ug-tile-image-overlay${imageEffectClassAdd}' >`;
            const imageEffectClass = ` ug-${g_options.tile_image_effect_type}-effect`;
        ...
        if (g_options.tile_enable_textpanel == true) {
            ...
            if (g_options.tile_textpanel_always_on == true) {
                const textPanelElement = getTextPanelElement(objThumbWrapper);
                textPanelElement.css('z-index', 2);
            }

            // if text panel is outside, clone textpanel
            if (g_temp.isTextpanelOutside == true) {
                const htmlClone = "<div class='ug-tile-cloneswrapper'></div>";
                objThumbWrapper.append(htmlClone);
                const objCloneWrapper = objThumbWrapper.children('.ug-tile-cloneswrapper');

                const objTextPanelClone = new UGTextPanel();
    ...
    this.loadTileImage = function (objTile) {
        const objImage = t.getTileImage(objTile);

        g_functions.checkImagesLoaded(objImage, null, (objImage, isError) => {
    ...
    function getTileOverlayImage(objTile) {
        const objOverlayImage = objTile.children('.ug-tile-image-overlay');
    ...
    function getTileOverlay(objTile) {
        const objOverlay = objTile.children('.ug-thumb-overlay');
        return (objOverlay);
    }
    ...
    function getTileImageContainer(objTile) {
        ...
        const objImageContainer = objTile.children('.ug-image-container');
    ...
    function getTileImageEffect(objTile) {
        const objImageEffect = objTile.find('.ug-tile-image-overlay img');
    ...
    function getTextPanel(objTile) {
        const objTextPanel = objTile.data('objTextPanel');
    ...
    function getTextPanelClone(objTile) {
        const objTextPanelClone = objTile.data('objTextPanelClone');
    ...
    function getTextPanelElement(objTile) {
        const objTextPanel = objTile.children('.ug-textpanel');
    ...
    function getTextPanelCloneElement(objTile) {
        const objTextPanel = objTile.find('.ug-tile-cloneswrapper .ug-textpanel');
    ...
    function getTextPanelHeight(objTile) {
        ...
        const objSize = g_functions.getElementSize(objTextPanel);
    ...
    function getButtonLink(objTile) {
        const objButton = objTile.find('.ug-icon-link');
    ...
    function getTileRatio(objTile, fromWhere) {
        // global ratio
        let ratio = g_temp.ratioByHeight;
    ...
    function getButtonZoom(objTile) {
        const objButton = objTile.find('.ug-button-play');
    ...
    function positionElements_images(objTile, width, height, visibleOnly) {
        const objImageOverlay = getTileOverlayImage(objTile);
        const objThumbImage = t.getTileImage(objTile);
        const objImageEffect = getTileImageEffect(objTile);
        ...
        if (g_temp.isTextpanelOutside == true) {
            const textHeight = getTextPanelHeight(objTile);
            height -= textHeight;
             ...
             */
            if (g_temp.hasImageContainer == true) {
                const objImageContainer = getTileImageContainer(objTile);
    ...
    function positionElements_textpanel(objTile, panelType, tileWidth, tileHeight) {
        ...
        if (panelType == 'clone') {
            const objTextPanelClone = getTextPanelClone(objTile);
            objTextPanelClone.refresh(true, true, panelWidth);
            const objItem = t.getItemByTile(objTile);
            objItem.textPanelCloneSizeSet = true;

            return (false);
        }

        const objTextPanel = getTextPanel(objTile);
        ...
        const isPosition = (g_options.tile_textpanel_always_on == true || g_options.tile_textpanel_appear_type == 'fade');

        if (isPosition) {
            if (g_temp.isTextpanelOutside == true && tileHeight && g_options.tile_textpanel_position == 'bottom') {
                const posy = tileHeight - panelHeight;
    ...
    function positionElements(objTile) {
        const objItem = t.getItemByTile(objTile);
        const objButtonZoom = getButtonZoom(objTile);
        const objButtonLink = getButtonLink(objTile);
        const sizeTile = g_functions.getElementSize(objTile);
        ...
        // position overlay:
        const overlayWidth = sizeTile.width - g_temp.tileInnerReduce;
        let overlayHeight = sizeTile.height - g_temp.tileInnerReduce;
        let overlayY = 0;
        if (g_temp.isTextpanelOutside == true) {
            const textHeight = getTextPanelHeight(objTile);
        ...
        const objOverlay = getTileOverlay(objTile);
        ...
        if (objButtonZoom || objButtonLink) {
            ...
            if (g_options.tile_enable_textpanel == true && g_temp.isTextPanelHidden == false && g_temp.isTextpanelOutside == false) {
                const objTextPanelElement = getTextPanelElement(objTile);
                const texPanelSize = g_functions.getElementSize(objTextPanelElement);
                if (texPanelSize.height > 0) { gapVert = Math.floor((texPanelSize.height / 2) * -1); }
            }
        }

        if (objButtonZoom && objButtonLink) {
            const sizeZoom = g_functions.getElementSize(objButtonZoom);
            const sizeLink = g_functions.getElementSize(objButtonLink);
            let spaceBetween = g_options.tile_space_between_icons;

            let buttonsWidth = sizeZoom.width + spaceBetween + sizeLink.width;
            let buttonsX = Math.floor((sizeTile.width - buttonsWidth) / 2);
    ...
    function setImageOverlayEffect(objTile, isActive) {
        const objItem = t.getItemByTile(objTile);
        const objOverlayImage = getTileOverlayImage(objTile);

        const animationDuration = g_options.thumb_transition_duration;
    ...
    function setTextpanelEffect(objTile, isActive) {
        ...
        const objTextPanel = getTextPanelElement(objTile);
        if (!objTextPanel) { return (true); }

        if (g_options.tile_textpanel_appear_type == 'slide') {
            const panelSize = g_functions.getElementSize(objTextPanel);
            if (panelSize.width == 0) { return (false); }

            const startPos = -panelSize.height;
            const endPos = 0;
            let startClass = {},
                endClass = {};
    ...
    function setIconsEffect(objTile, isActive, noAnimation) {
        ...
        const g_objIconZoom = getButtonZoom(objTile);
        const g_objIconLink = getButtonLink(objTile);
    ...
    function setNormalStyle(data, objTile) {
        ...
        if (g_temp.isSaparateIcons == true && g_options.tile_enable_icons == true) {
            ...
            const objItem = t.getItemByTile(objTile);
            if (isItemIconAlwaysOn(objItem) == false) { setIconsEffect(objTile, isSet, false); } else {	// make icon always appear
    ...
    function setAllTilesNormalStyle(objTileExcept) {
        const objTiles = g_thumbs.getThumbs().not(objTileExcept);
        objTiles.each((index, objTile) => {
    ...
    function onTileClick(event) {
        ...
        const tagname = objTile.prop('tagName').toLowerCase();
        let isApproved = true;
    ...
    function onZoomButtonClick(event) {
        ...
        const objTile = jQuery(this).parents('.ug-tile');
    ...
    this.destroy = function () {
        ...
        if (g_options.tile_enable_textpanel == true) {
            const objThumbs = g_thumbs.getThumbs();
            jQuery.each(objThumbs, (index, thumb) => {
                const textPanel = getTextPanel(jQuery(thumb));
    ...
    this.getTileImage = function (objTile) {
        const objImage = objTile.find('img.ug-thumb-image');
        return (objImage);
    };
    ...
    this.getTileHeightByWidth = function (newWidth, objTile, fromWhere) {
        const ratio = getTileRatio(objTile, fromWhere);

        if (ratio === null) { return (null); }

        let height = Math.floor((newWidth - g_temp.tileInnerReduce) * ratio) + g_temp.tileInnerReduce;
...
function UGAviaControl() {
    ...
    function initEvents(event) {
        ...
        jQuery('body').on('touchstart', (event) => {
            if (g_temp.isControlEnabled == false) { return (true); }
        ...
        jQuery('body').mousemove((event) => {
            ...
            const strip_touch_active = g_parent.isTouchMotionActive();
    ...
    function getInnerPosY(mouseY) {
        const innerOffsetTop = g_options.strip_padding_top;
        const innerOffsetBottom = g_options.strip_padding_bottom;

        const stripHeight = g_objStrip.height();
        const innerHeight = g_objStripInner.height();

        // if all thumbs visible, no need to move
        if (stripHeight > innerHeight) { return (null); }

        // find y position inside the strip
        const stripOffset = g_objStrip.offset();
        const offsetY = stripOffset.top;
        let posy = mouseY - offsetY - innerOffsetTop;
        if (posy < 0) { return (null); }

        // set measure line parameters
        const mlineStart = g_options.thumb_height;
        const mlineEnd = stripHeight - g_options.thumb_height;
        const mLineSize = mlineEnd - mlineStart;
        ...
        const ratio = (posy - mlineStart) / mLineSize;
        let innerPosY = (innerHeight - stripHeight) * ratio;
    ...
    function getInnerPosX(mouseX) {
        const innerOffsetLeft = g_options.strip_padding_left;
        const innerOffsetRight = g_options.strip_padding_right;

        const stripWidth = g_objStrip.width() - innerOffsetLeft - innerOffsetRight;
        const innerWidth = g_objStripInner.width();

        // if all thumbs visible, no need to move
        if (stripWidth > innerWidth) { return (null); }

        const stripOffset = g_objStrip.offset();
        const offsetX = stripOffset.left;
        let posx = mouseX - offsetX - innerOffsetLeft;

        // set measure line parameters
        const mlineStart = g_options.thumb_width;
        const mlineEnd = stripWidth - g_options.thumb_width;
        const mLineSize = mlineEnd - mlineStart;
        ...
        // count the ratio of the position on the measure line
        const ratio = (posx - mlineStart) / mLineSize;
        let innerPosX = (innerWidth - stripWidth) * ratio;
    ...
    function moveStripStep() {
        ...
        const innerPos = g_parent.getInnerStripPos();
        ...
        // calc step
        const diff = Math.abs(g_temp.strip_finalPos - innerPos);
        ...
        if (g_temp.strip_finalPos < innerPos) { dpos *= -1; }

        const newPos = innerPos + dpos;
    ...
    function getInnerPos(mousePos) {
        if (g_isVertical == false) { return getInnerPosX(mousePos); }
    ...
    function moveStripToMousePosition(mousePos) {
        const innerPos = getInnerPos(mousePos);
...
function UGSlider() {
    ...
    let g_objSlider,
        ...
        g_objArrowLeft,
        g_objArrowRight;
    let g_objTouchSlider,
        g_objZoomSlider,
        g_objZoomPanel,
        g_objButtonPlay = null,
        g_objButtonFullscreen = null,
        g_objBullets = null;
    let g_objVideoPlayer = new UGVideoPlayer(),
        g_optionsPrefix;
    let g_functions = new UGFunctions(),
        g_objProgress = null,
        g_objTextPanel = null;
    ...
    function initSlider(objGallery, objOptions, optionsPrefix) {
        ...
        const galleryID = g_gallery.getGalleryID();
    ...
    function getHtmlSlide(loaderClass, numSlide) {
        let classVideoplay = 'ug-type-square';
    ...
    function setHtmlSlider(objParent) {
        ...
        let html = "<div class='ug-slider-wrapper'>";
        ...
        if (g_options.slider_enable_progress_indicator == true) {
            ...
            const finalType = g_objProgress.getType();
        ...
        if (g_options.slider_enable_text_panel == true) {
            ...
            if (g_options.slider_textpanel_always_on == false) {
                // hide the panel
                const panelElement = g_objTextPanel.getElement();
    ...
    function positionSlideElements(objSlide) {
        // position preloader
        const objPreloader = getSlidePreloader(objSlide);
        g_functions.placeElementInParentCenter(objPreloader);

        // position video play button
        const objVideoPlayButton = getSlideVideoPlayButton(objSlide);
    ...
    function positionElements() {
        ...
        if (g_objProgress) {
            const objProgressElement = g_objProgress.getElement();

            if (g_options.slider_progress_indicator_type == 'bar') {
                const sliderWidth = g_objSlider.width();
    ...
    function placeControlsElements() {
        ...
        // position zoom panel
        if (g_objZoomPanel) {
            const zoomPanelElement = g_objZoomPanel.getElement();
    ...
    function positionSlides() {
        const slides = t.getSlidesReference();
        let posX = 0,
            posY = 0,
            innerWidth;
        let posXPrev = 0,
    ...
    function resizeSlideItem(objSlide) {
        const index = objSlide.data('index');
    ...
    function getImageBorderWidth(objImage, imageData) {
        ...
        // take the less size
        const totalSize = (imageWidth < imageHeight) ? imageWidth : imageHeight;
        const borderSize = borderWidth * 2;

        const borderRatio = borderSize / totalSize;
    ...
    function setImageDesign(objImage, slideType, imageData) {
        ...
        if (g_options.slider_image_border == true) {
            ...
            const borderWidth = getImageBorderWidth(objImage, imageData);
    ...
    function scaleImageConstantSize(objImage, objItem) {
        const constantWidth = g_options.slider_video_constantsize_width;
        const constantHeight = g_options.slider_video_constantsize_height;
        const scaleMode = g_options.slider_video_constantsize_scalemode;

        const objSize = g_functions.scaleImageExactSizeInParent(objImage, objItem.imageWidth, objItem.imageHeight, constantWidth, constantHeight, scaleMode);
    ...
    function setImageToSlide(objSlide, objItem, isForce) {
        const objItemWrapper = objSlide.children('.ug-item-wrapper');
        ...
        const urlImage = objItem.urlImage;

        const currentImage = objSlide.data('urlImage');

        objSlide.data('urlImage', urlImage);

        const scaleMode = t.getScaleMode(objSlide);

        const slideType = t.getSlideType(objSlide);
        ...
        if (currentImage == urlImage && isForce !== true) {
            ...
            if (objItem.isBigImageLoaded == true) {
                ...
                objImage.on('load', function () {
                    ...
                    const objImage = jQuery(this);
                    const itemIndex = objImage.data('itemIndex');

                    objImage.fadeTo(0, 1);

                    // get and hide preloader
                    const objSlide = objImage.parent().parent();
                    const slideType = t.getSlideType(objSlide);
                    const objPreloader = getSlidePreloader(objSlide);
                    const objPadding = t.getObjImagePadding();
                    const scaleMode = t.getScaleMode(objSlide);
                    ...
                    const objItem = g_gallery.getItem(itemIndex);
    ...
    function setItemToSlide(objSlide, objItem) {
        try {
            ...
            const currentIndex = objSlide.data('index');
    ...
    function isTextPanelHidden() {
        ...
        const isHidden = panelElement.data('isHidden');
    ...
    function setVideoPlayerPosition() {
        const objCurrentSlide = t.getCurrentSlide();
        const objImage = t.getSlideImage(objCurrentSlide);

        const slideSize = g_functions.getElementSize(objCurrentSlide);
        let left = slideSize.left;
        let top = slideSize.top;
    ...
    function setVideoPlayerConstantSize() {
        const videoWidth = g_options.slider_video_constantsize_width;
        const videoHeight = g_options.slider_video_constantsize_height;

        g_objVideoPlayer.setSize(videoWidth, videoHeight);

        // set video design
        const videoElement = g_objVideoPlayer.getObject();
    ...
    function doTransition(direction, objItem, forceTransition) {
        ...
        let transition = g_options.slider_transition;
    ...
    function transitionSlide(direction, objItem) {
        ...
        const transSpeed = g_options.slider_transition_speed;
        const transEasing = g_options.slider_transition_easing;
        ...
        const animateParams = {
            ...
            always() {
                ...
                if (g_temp.itemWaiting != null) {
                    const direction = getSlideDirection(g_temp.itemWaiting);
    ...
    function transitionFade(objItem, noAnimation, noHidePlayer) {
        ...
        const objCurrentPos = g_functions.getElementSize(slides.objCurrentSlide);

        g_functions.placeElement(slides.objNextSlide, objCurrentPos.left, objCurrentPos.top);

        // switch slide nums
        const currentNum = g_temp.numCurrent;
        g_temp.numCurrent = g_temp.numNext;
        g_temp.numNext = currentNum;
        ...
        if (noAnimation == true) {
            ...
            animateOpacity(slides.objCurrentSlide, 0, () => {
                t.placeNabourItems();
                g_objThis.trigger(t.events.TRANSITION_END);
                if (noHidePlayer !== true) { g_objVideoPlayer.hide(); }
            });
    ...
    function getControlsObjects() {
        const objControl = g_objSlider.children('.ug-slider-control');
    ...
    function hideControls(noAnimation) {
        ...
        const objControls = getControlsObjects();

        if (noAnimation === true) { objControls.stop().fadeTo(0, 0).hide(); } else {
            objControls.stop().fadeTo(g_options.slider_controls_appear_duration, 0, () => { objControls.hide(); });
        }
    ...
    function onActualClick() {
        // check link
        const currentSlide = t.getCurrentSlide();
        const isClickable = currentSlide.hasClass('ug-slide-clickable');
        const objItem = t.getCurrentItem();
    ...
    function objVideoPlayClick(objButton) {
        const objSlide = objButton.parent();
    ...
    function onSlideImageLoaded(data, objSlide) {
        ...
        const objImage = t.getSlideImage(objSlide);
        const objButtonVideoPlay = getSlideVideoPlayButton(objSlide);
        const objSize = g_functions.getElementSize(objImage);
    ...
    function initEvents() {
        ...
        g_objSlider.delegate('.ug-item-wrapper img', 'mouseenter', (event) => {
            g_objThis.trigger(t.events.IMAGE_MOUSEENTER);
        });

        g_objSlider.delegate('.ug-item-wrapper img', 'mouseleave', (event) => {
            const isMouseOver = t.isMouseInsideSlideImage(event);
    ...
    function getSlideDirection(objItem) {
        ...
        const currentIndex = slides.objCurrentSlide.data('index');
        const nextIndex = objItem.index;
    ...
    function getSlidePreloader(objSlide) {
        ...
        const objPreloader = objSlide.children('.ug-slider-preloader');
        return (objPreloader);
    }
    ...
    function getSlideVideoPlayButton(objSlide) {
        const objButton = objSlide.children('.ug-button-videoplay');
        return (objButton);
    }
    ...
    function getNumSlide(objSlide) {
        const numSlide = objSlide.data('slidenum');
        return (numSlide);
    }
    ...
    this.getSlideImage = function (objSlide) {
        ...
        const objImage = objSlide.find('.ug-item-wrapper img');
        return (objImage);
    };
    ...
    this.isInnerInPlace = function () {
        ...
        const posCurrent = g_functions.getElementSize(slides.objCurrentSlide);
        const inPlaceX = -posCurrent.left;
        const objInnerSize = g_functions.getElementSize(g_objInner);

        if (inPlaceX == objInnerSize.left) { return (true); }
        return (false);
    };
    ...
    this.isAnimating = function () {
        const isAnimated = g_objInner.is(':animated');
    ...
    this.getCurrentItemIndex = function () {
        ...
        let currentIndex = slides.objCurrentSlide.data('index');
    ...
    this.getCurrentItem = function () {
        const currentIndex = t.getCurrentItemIndex();
        if (currentIndex == -1) { return (null); }

        const objItem = g_gallery.getItem(currentIndex);
    ...
    this.isMouseInsideSlideImage = function (event) {
        const objImage = t.getSlideImage();

        let point = g_functions.getMousePosition(event);
        if (point.pageX === undefined) { point = g_objTouchSlider.getLastMousePos(); }

        const pointImg = g_functions.getMouseElementPoint(point, objImage);
        const objSize = g_functions.getElementSize(objImage);
    ...
    this.isCurrentSlideType = function (type) {
        const currentSlideType = t.getSlideType();
    ...
    this.isCurrentSlideLoadingImage = function () {
        const currentSlide = t.getCurrentSlide();
        const isLoading = currentSlide.data('isLoading');
    ...
    this.setItem = function (objItem, forseTransition, role) {
        ...
        const isFirstSlide = (currentIndex == undefined);

        if (isFirstSlide) {
            setItemToSlide(slides.objCurrentSlide, objItem);
            t.placeNabourItems();
        } else {
            let direction = 'left';		// move foreward
    ...
    this.placeNabourItems = function () {
        ...
        const itemPrev = g_gallery.getPrevItem(currentIndex);
        const itemNext = g_gallery.getNextItem(currentIndex);
    ...
    this.startSlideAction = function (objSlide) {
        ...
        const objItem = getSlideItem(objSlide);
    ...
    this.getScaleMode = function (objSlide) {
        ...
        if (g_gallery.isFullScreen() == true) { return (g_options.slider_scale_mode_fullscreen); }
    ...
    this.isCurrentSlideImageFit = function () {
        const objSlide = t.getCurrentSlide();
        ...
        const isFit = g_functions.isImageFitParent(objImage);
    ...
    this.isCurrentImageInPlace = function () {
        ...
        const scaleMode = t.getScaleMode();
        const objPadding = t.getObjImagePadding();
        const objItem = getSlideItem();

        const objParent = objImage.parent();

        const objFitSize = g_functions.getImageInsideParentData(objParent, objItem.imageWidth, objItem.imageHeight, scaleMode, objPadding);
        const objSize = g_functions.getElementSize(objImage);
    ...
    this.isSwiping = function () {
        ...
        const isActive = g_objTouchSlider.isTouchActive();
    ...
    this.isPreloading = function () {
        const objPreloader = getSlidePreloader();
    ...
    this.setSize = function (width, height) {
        ...
        const itemWidth = width - g_options.slider_item_padding_left - g_options.slider_item_padding_right;
        const itemHeight = height - g_options.slider_item_padding_top - g_options.slider_item_padding_bottom;

        // set item wrapper
        const objCssItemWrapper = {};
        ...
        if (currentSlideType != 'image' && g_options.slider_video_constantsize == true) {
            ...
            const videoWidth = width - g_options.slider_video_padding_left - g_options.slider_video_padding_right;
            const videoHeight = height - g_options.slider_video_padding_top - g_options.slider_video_padding_bottom;
...
function UGTextPanel() {
    ...
    function positionElementsTop(animateHeight, startY) {
        ...
        if (g_objTitle) {
            ...
            const objTitleSize = g_functions.getElementSize(g_objTitle);
        ...
        if (textDesc != '') {
            ...
            const objDescSize = g_functions.getElementSize(g_objDesc);
            maxy = objDescSize.bottom;
        }

        // change panel height
        if (!g_options.textpanel_height && g_temp.setInternalHeight == true) {
            const panelHeight = maxy + g_options.textpanel_padding_bottom;
    ...
    function positionElementsMiddle() {
        const totalTextHeight = getTotalTextHeight();
        const startY = (g_objTextWrapper.height() - totalTextHeight) / 2;
    ...
    function positionElementBottom() {
        const totalTextHeight = getTotalTextHeight();
        const startY = g_objTextWrapper.height() - totalTextHeight - g_options.textpanel_padding_bottom;
    ...
    function setHeight(height, animateHeight) {
        ...
        if (animateHeight == true) {
            if (g_objBG) {
                ...
                const currentHeight = g_objBG.height();
                if (height > currentHeight) { g_objBG.height(height); }
            }

            const objCss = { height: `${height}px` };
    ...
    this.appendHTML = function (objParent, addClass) {
        ...
        let html = `<div class='ug-textpanel${addClass}'>`;
    ...
    function setCss() {
        ...
        if (g_options.textpanel_enable_bg == true) {
            ...
            let objCssBG = { 'background-color': g_options.textpanel_bg_color };
    ...
    this.setPanelSize = function (panelWidth, panelHeight) {
        ...
        if (!panelHeight) { var panelHeight = 80; }		// some default number
        else { g_temp.setInternalHeight = false; }
        ...
        const textWrapperWidth = panelWidth - g_options.textpanel_padding_left - g_options.textpanel_padding_right;
        const textWrapperLeft = g_options.textpanel_padding_left;
    ...
    this.setSizeByParent = function () {
        const objSize = g_functions.getElementSize(g_objParent);
...
function UGZoomButtonsPanel() {
    ...
    this.appendHTML = function (objParent) {
        ...
        let html = `<div class='ug-slider-control ug-zoompanel ug-skin-${g_options.slider_zoompanel_skin}'>`;
    ...
    function onZoomChange() {
        ...
        const isFit = g_slider.isCurrentSlideImageFit();
    ...
    t.initEvents = function () {
        ...
        g_functions.setButtonOnClick(g_objButtonPlus, () => {
            if (isButtonDisabled(g_objButtonPlus) == true) { return (true); }

            g_slider.zoomIn();
        });

        g_functions.setButtonOnClick(g_objButtonMinus, () => {
            if (isButtonDisabled(g_objButtonMinus) == true) { return (true); }

            g_slider.zoomOut();
        });

        g_functions.setButtonOnClick(g_objButtonReturn, () => {
...
function UGBullets() {
    ...
    function setHtmlBullets() {
        ...
        if (!g_bulletWidth) {
            const objBullet = g_objBullets.find('.ug-bullet:first-child');
    ...
    this.getBulletsWidth = function () {
        ...
        const totalWidth = g_numBullets * g_bulletWidth + (g_numBullets - 1) * g_options.bullets_space_between;
        return (totalWidth);
    };
    ...
    this.appendHTML = function (objParent) {
        ...
        let html = `<div class='ug-slider-control ug-bullets ug-skin-${g_options.bullets_skin}${addClass}'>`;
     ...
     */
    function initEvents() {
        const objBullets = g_objBullets.children('.ug-bullet');

        g_functions.setButtonOnClick(objBullets, onBulletClick);

        objBullets.on('mousedown mouseup', event =>
            // event.preventDefault();
            (false),
        );
    }
    ...
    this.setActive = function (index) {
        ...
        const children = g_objBullets.children('.ug-bullet');
        children.removeClass('ug-bullet-active');

        const bullet = jQuery(children[index]);
...
function UGProgressBar() {
    ...
    this.put = function (g_objWrapper, userOptions) {
        ...
        const opacity = g_options.slider_progressbar_opacity;

        const objInnerHTML = g_objInner[0];
     ...
     */
    this.draw = function () {
        const innerWidth = g_percent * 100;
...
function UGProgressPie() {
    ...
    function draw(percent) {
        ...
        const radius = Math.min(g_options.slider_progresspie_width, g_options.slider_progresspie_height) / 2;

        const ctx = g_objPie[0].getContext('2d');
        ...
        const centerX = g_options.slider_progresspie_width / 2;
        const centerY = g_options.slider_progresspie_height / 2;

        // draw main arc
        let startPoint = 0;
        let endPoint = percent * Math.PI * 2;
        ...
        if (g_options.slider_progresspie_type_fill == true) {		// fill
            ...
            const radius2 = radius - g_options.slider_progresspie_stroke_width;
...
function UGTouchSliderControl() {
    ...
    function getDiffPosFromCurrentItem(slides) {
        ...
        const inPlaceX = -posCurrent.left;
        const objInnerSize = g_functions.getElementSize(g_objInner);
        const diffPos = inPlaceX - objInnerSize.left;
     ...
     */
    function isMovementValidForChange() {
        const slides = g_parent.getSlidesReference();

        // check position, if more then half, move
        const diffPos = getDiffPosFromCurrentItem(slides);

        const breakSize = Math.round(slides.objCurrentSlide.width() * 3 / 8);

        if (Math.abs(diffPos) >= breakSize) { return (true); }

        // check gesture, if vertical mostly then not move
        const diffX = Math.abs(g_temp.lastMouseX - g_temp.startMouseX);
        const diffY = Math.abs(g_temp.lastMouseY - g_temp.startMouseY);
        ...
        const endTime = jQuery.now();
        const diffTime = endTime - g_temp.startTime;
    ...
    this.isTapEventOccured = function (event) {
        // validate one touch
        const arrTouches = g_functions.getArrTouches(event);
        const numTouches = arrTouches.length;
    ...
    function returnToPlace(slides) {
        ...
        g_objInner.animate({ left: `${destX}px` }, {
            ...
            queue: false,
            progress(animation, number, remainingMS) {
                // check drag video
                if (g_temp.isDragVideo == true) {
                    const objSize = g_functions.getElementSize(g_objInner);
                    const innerX = objSize.left;

                    const posDiff = innerX - destX;

                    const videoPosX = g_temp.videoStartX + posDiff;
    ...
    function continueSlideDragChange() {
        ...
        const direction = (diffPos > 0) ? 'left' : 'right';
        ...
        switch (direction) {
            case 'right':	// change to prev item
                ...
                if (g_parent.isSlideHasItem(slides.objPrevSlide)) {
                    const posPrev = g_functions.getElementSize(slides.objPrevSlide);
                    var destX = -posPrev.left;
            ...
            case 'left':		// change to next item
                ...
                if (g_parent.isSlideHasItem(slides.objNextSlide)) {
                    const posNext = g_functions.getElementSize(slides.objNextSlide);
                    var destX = -posNext.left;
                } else { isReturn = true; }
                break;
        }
            ...
            g_objInner.stop().animate({ left: `${destX}px` }, {
                ...
                progress() {
                    ...
                    if (g_temp.isDragVideo == true) {
                        const objSize = g_functions.getElementSize(g_objInner);
                        const innerX = objSize.left;
                        const posDiff = innerX - g_temp.startPosx;
                        const videoPosX = g_temp.videoStartX + posDiff;
                        g_temp.videoObject.css('left', videoPosX);
                    }
                },
    ...
    function handleSliderDrag(event) {
        const diff = g_temp.lastMouseX - g_temp.startMouseX;

        if (diff == 0) { return (true); }

        const direction = (diff < 0) ? 'left' : 'right';

        const objZoomSlider = g_parent.getObjZoom();

        // don't drag if the zoom panning enabled
        // store init position after image zoom pan end
        if (objZoomSlider) {
            const isPanEnabled = objZoomSlider.isPanEnabled(event, direction);
        ...
        let currentPosx = g_temp.startPosx + diff;

        // check out of borders and slow down the motion:
        if (diff > 0 && currentPosx > 0) { currentPosx /= 3; } else if (diff < 0) {
            const innerEnd = currentPosx + g_objInner.width();
            const sliderWidth = g_objSlider.width();
        ...
        // drag video
        if (g_temp.isDragVideo == true) {
            const posDiff = currentPosx - g_temp.startPosx;
            const videoPosX = g_temp.videoStartX + posDiff;
    ...
    function storeInitTouchData(event) {
        ...
        const objPos = g_functions.getElementSize(g_objInner);
    ...
    function onTouchEnd(event) {
        ...
        const arrTouches = g_functions.getArrTouches(event);
        const numTouches = arrTouches.length;
        const isParentInPlace = g_parent.isInnerInPlace();
        ...
        if (numTouches == 0 && g_temp.touch_active == true) {
            ...
            const wasVerticalScroll = g_functions.wasVerticalScroll(g_temp.storedEventID);

            if (wasVerticalScroll == false) { isValid = isMovementValidForChange(); }

            if (isValid == true) { continueSlideDragChange(); }		// change the slide
            else { returnToPlace(); }	// return the inner object to place (if not in place)
...
function UGZoomSliderControl() {
    ...
    function getFitScaleMode() {
        let scaleMode = g_parent.getScaleMode();
    ...
    function storeCurrentImage() {
        ...
        const diff = currentTime - g_temp.storeImageLastTime;
        ...
        const scaleMode = getFitScaleMode();
    ...
    function zoomCurrentImage(mode, mousePos) {
        const slides = g_parent.getSlidesReference();
        const objImage = slides.objCurrentSlide.find('img');
        const scaleMode = getFitScaleMode();
        ...
        const objPadding = g_parent.getObjImagePadding();

        if (mode == 'back') {
            const objOriginalSize = g_functions.getImageOriginalSize(objImage);
    ...
    function isPanPosible(objImage, event, stictTouchesCheck) {
        ...
        } else if (arrTouches.length > 1) { return (false); }
    ...
    function panImage(event) {
        ...
        const diffX = mousePos.pageX - g_temp.startMouseX;
        const diffY = mousePos.pageY - g_temp.startMouseY;

        // get active direction
        let diffLastX = mousePos.pageX - g_temp.lastMouseX;
        let diffLastY = mousePos.pageY - g_temp.lastMouseY;

        const directionX = (diffLastX < 0) ? 'left' : 'right';
        const directionY = (diffLastY < 0) ? 'up' : 'down';

        g_temp.lastMouseX = mousePos.pageX;
        g_temp.lastMouseY = mousePos.pageY;

        const posImage = g_functions.getElementSize(g_temp.objImage);
        ...
        const imageX = posImage.left + diffLastX;
        const imageY = posImage.top + diffLastY;
    ...
    function checkReturnAfterPan() {
        let isReturnX = false,
            ...
            newX = 0,
            newY = 0;
        const objSize = g_functions.getElementSize(g_temp.objImage);
        const objImagePadding = g_parent.getObjImagePadding();

        const objCenterPos = g_functions.getElementCenterPosition(g_temp.objImage, objImagePadding);
    ...
    function startTouchZoom(arrTouches) {
        ...
        const isInsideImage = g_functions.isPointInsideElement(g_temp.imageOrientPoint, g_temp.objImageSize);
    ...
    function doTouchZoom(event) {
        ...
        const distance = g_functions.getDistance(arrTouches[0].pageX, arrTouches[0].pageY, arrTouches[1].pageX, arrTouches[1].pageY);
        const zoomRatio = distance / g_temp.startDistance;

        const middlePoint = g_functions.getMiddlePoint(arrTouches[0].pageX, arrTouches[0].pageY, arrTouches[1].pageX, arrTouches[1].pageY);

        // set zoom data:
        const newWidth = g_temp.objImageSize.width * zoomRatio;
        const newHeight = g_temp.objImageSize.height * zoomRatio;

        // check max zoom ratio:
        const objOriginalSize = g_functions.getImageOriginalSize(g_temp.objImage);
        let expectedZoomRatio = 1;
        if (objOriginalSize.width > 0) { expectedZoomRatio = newWidth / objOriginalSize.width; }
        ...
        const diffMiddleX = (middlePoint.x - g_temp.startMiddlePoint.x);
        const diffMiddleY = (middlePoint.y - g_temp.startMiddlePoint.y);

        const posx = g_temp.startImageX + panX + diffMiddleX;
        const posy = g_temp.startImageY + panY + diffMiddleY;
    ...
    function onTouchStart(event) {
        ...
        const isStored = storeCurrentImage();
    ...
    function onTouchEnd(event) {
        ...
        const objTarget = jQuery(event.target);
        ...
        const panWasActive = g_temp.isPanActive;
        const zoomWasActive = g_temp.isZoomActive;
        ...
        if (g_temp.isZoomActive == true) {
            ...
            const panPosible = isPanPosible(g_temp.objImage, event, true);
    ...
    function initEvents() {
        ...
        g_objParent.bind(g_parent.events.BEFORE_RETURN, () => {
            checkReturnFromZoom();
        });

        // on item change update isZoomedOnce event. Allow panning only if zoomed once
        g_objParent.bind(g_parent.events.ITEM_CHANGED, () => {
            g_temp.isZoomedOnce = false;
        });

        g_objParent.bind(g_parent.events.ZOOM_CHANGE, () => {
...
function UGWistiaAPI() {
    ...
    function putVideoActually(divID, videoID, width, height, isAutoplay) {
        ...
        const htmlID = `${divID}_video`;

        const html = `<div id='${htmlID}' class='wistia_embed' style='width:${width};height:${height};' data-video-width='${width}' data-video-height='${height}'>&nbsp;</div>`;
...
function UGSoundCloudAPI() {
    ...
    function putSoundActually(divID, trackID, width, height, isAutoplay) {
        ...
        const iframeID = `${divID}_iframe`;

        let url = `${location.protocol}//w.soundcloud.com/player/?url=http://api.soundcloud.com/tracks/${trackID}`;
        url += '&amp;buying=false&amp;liking=false&amp;download=false&amp;sharing=false&amp;show_artwork=true&show_comments=false&amp;show_playcount=true&amp;show_user=false&amp;hide_related=true&amp;visual=true&amp;start_track=0&amp;callback=true';

        if (isAutoplay === true) { url += '&amp;auto_play=true'; } else { url += '&amp;auto_play=false'; }

        const html = `<iframe id='${iframeID}' src=${url} width='${width}' height='${height}' frameborder='0' scrolling='no' webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>`;
        ...
        g_player.bind(SC.Widget.Events.READY, () => {
            if (g_player) {
                g_isPlayerReady = true;
                initEvents();
            }
    ...
    function initEvents() {
        // set "play" event
        g_player.bind(SC.Widget.Events.PLAY, () => {
            g_objThis.trigger(t.events.START_PLAYING);
        });

        // set "pause event"
        g_player.bind(SC.Widget.Events.PAUSE, () => {
            g_objThis.trigger(t.events.STOP_PLAYING);
        });

        g_player.bind(SC.Widget.Events.FINISH, () => {
            g_objThis.trigger(t.events.STOP_PLAYING);
            g_objThis.trigger(t.events.VIDEO_ENDED);
        });
    }
...
function UGHtml5MediaAPI() {
    ...
    function putVideoActually(divID, data, width, height, isAutoplay) {
        ...
        const urlFlash = `${location.protocol}//cdnjs.cloudflare.com/ajax/libs/mediaelement/2.18.1/flashmediaelement-cdn.swf`;
        const urlSilverlight = `${location.protocol}//cdnjs.cloudflare.com/ajax/libs/mediaelement/2.18.1/silverlightmediaelement.xap`;
        ...
        let html = `<video id='${htmlID}' width='${width}' height='${height}'  controls='controls' preload='none' ${htmlAutoplay} ${htmlPoster}>`;
     ...
     */
    function initEvents() {
        g_ugFunctions.addEvent(g_player, 'play', () => {
            g_objThis.trigger(t.events.START_PLAYING);
        });

        g_ugFunctions.addEvent(g_player, 'pause', () => {
            g_objThis.trigger(t.events.STOP_PLAYING);
        });

        g_ugFunctions.addEvent(g_player, 'ended', () => {
            g_objThis.trigger(t.events.STOP_PLAYING);
            g_objThis.trigger(t.events.VIDEO_ENDED);
        });
    }
...
function UGVimeoAPI() {
    ...
    function putVideoActually(divID, videoID, width, height, isAutoplay) {
        ...
        let url = `${location.protocol}//player.vimeo.com/video/${videoID}?api=1`;

        if (isAutoplay === true) { url += '&amp;byline=0&amp;autoplay=1&amp;title=0&amp;portrait=0'; }

        const html = `<iframe src=${url} width='${width}' height='${height}' frameborder='0' webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>`;

        jQuery(`#${divID}`).html(html);

        // get the player object
        const iframe = jQuery(`#${divID} iframe`)[0];

        g_player = Froogaloop(iframe);

        g_player.addEvent('ready', () => {
            if (g_player) {
                g_isPlayerReady = true;
                initEvents();
            }
    ...
    function initEvents() {
        ...
        // set "play" event
        g_player.addEvent('play', () => {
            g_objThis.trigger(t.events.START_PLAYING);
        });

        // set "pause event"
        g_player.addEvent('pause', () => {
            g_objThis.trigger(t.events.STOP_PLAYING);
        });

        g_player.addEvent('finish', () => {
            g_objThis.trigger(t.events.STOP_PLAYING);
            g_objThis.trigger(t.events.VIDEO_ENDED);
        });
    }
    ...
    this.getVideoImages = function (videoID, itemIndex, onSuccessFunction) {
        const url = `${location.protocol}//vimeo.com/api/v2/video/${videoID}.json`;
        jQuery.get(url, {}, (data) => {
            const obj = {};
...
function UGYoutubeAPI() {
    ...
    function onPlayerStateChange() {
        ...
        const state = g_player.getPlayerState();
...
function UGVideoPlayer() {
    let t = this,
        ...
        g_objThis = jQuery(this),
        g_functions = new UGFunctions();
    let g_youtubeAPI = new UGYoutubeAPI(),
        g_vimeoAPI = new UGVimeoAPI();
    let g_html5API = new UGHtml5MediaAPI(),
    ...
    this.setHtml = function (objParent) {
        ...
        let html = "<div class='ug-videoplayer' style='display:none'>";
     ...
     */
    this.pause = function () {
        const activeAPI = t.getActiveAPI();
    ...
    function stopAndHidePlayers(except) {
        const arrPlayers = ['youtube', 'vimeo', 'html5', 'soundcloud', 'wistia'];
        for (const index in arrPlayers) {
    ...
    this.playYoutube = function (videoID, isAutoplay) {
        ...
        const objYoutubeInner = g_objYoutube.children(`#${g_temp.youtubeInnerID}`);
...
jQuery.fn.unitegallery = function (options) {
    ...
    const objGallery = new UniteGalleryMain();
    objGallery.run(galleryID, options);

    const api = new UG_API(objGallery);
...
function ugCheckForMinJQueryVersion() {
    const isMinJQuery = g_ugFunctions.checkMinJqueryVersion('1.8.0');
...
function ugCheckForErrors(galleryID, type) {
    ...
    function checkForDoubleJQuery() {
        ...
        let errorMessage = 'You have some jquery.js library include that comes after the gallery files js include.';
    ...
    try {
        ...
        let message = objError.message;
        message = `Unite Gallery Error: ${message}`;
        message = `<div style='font-size:16px;color:#BC0C06;max-width:900px;border:1px solid red;padding:10px;'>${message}</div>`;

        if (type == 'jquery') {
            const objGallery = document.getElementById(galleryID);
...
function UniteGalleryMain() {
    ...
    function checkForStartupErrors() {
        ...
        setTimeout(() => { ugCheckForErrors(g_galleryID, 'cms'); }, 5000);
    }
    ...
    function runGallery(galleryID, objCustomOptions, htmlItems, cacheID) {
        const isCustomOptions = (typeof objCustomOptions === 'object');
        ...
        const galleryWidth = g_objWrapper.width();
    ...
    function showErrorMessage(message, prefix) {
        ...
        const html = `<div class='ug-error-message-wrapper'><div class='ug-error-message'>${message}</div></div>`;
    ...
    function clearInitData() {
        const objItems = g_objWrapper.children().remove();
    }
    ...
    function setHeightByOriginalRatio() {
        ...
        const ratio = objSize.width / objSize.height;

        if (ratio != objSize.orig_ratio) {
            let newHeight = objSize.width / objSize.orig_ratio;
    ...
    function setHtmlObjectsProperties() {
        const optionWidth = g_functions.getCssSizeParam(g_options.gallery_width);
        ...
        if (g_temp.isFreestyleMode == false) {
            const galleryHeight = g_functions.getCssSizeParam(g_options.gallery_height);
    ...
    function fillItemsArray(arrChildren) {
        ...
        const isMobile = t.isMobileMode();

        let numIndex = 0;

        for (let i = 0; i < arrChildren.length; i++) {
            let objChild = jQuery(arrChildren[i]);
            ...
            let itemType = objChild.data('type');
            ...
            if (tagname == 'img') {
                ...
                const lasyLoadSrc = objChild.data('lazyload-src');
                ...
                // src is thumb
                let urlImage = objChild.data('image');
                let urlThumb = objChild.data('thumb');

                if (typeof (urlImage) === 'undefined') { urlImage = null; }

                if (typeof (urlThumb) === 'undefined') { urlThumb = null; }

                const imageSrc = objChild.attr('src');
            ...
            if (isMobile == true) {
                const urlThumbMobile = objChild.data('thumb-mobile');
                ...
                const urlImageMobile = objChild.data('image-mobile');
            ...
            const isImageMissing = (objItem.urlImage == undefined || objItem.urlImage == '');
            const isThumbMissing = (objItem.urlThumb == undefined || objItem.urlThumb == '');
            ...
            switch (objItem.type) {
                case 'youtube':
                    ...
                    if (isImageMissing || isThumbMissing) {
                        const objImages = g_ugYoutubeAPI.getVideoImages(objItem.videoid);
    ...
    function preloadBigImages() {
        ...
        switch (g_options.gallery_images_preload_type) {
            ...
            case 'visible':
                jQuery(g_arrItems).each(function () {
                    const objItem = this;
                    const isVisible = g_objThumbs.isItemThumbVisible(objItem);
    ...
    function preloadItemImage(objItem) {
        ...
        const imageUrl = objItem.urlImage;
        ...
        objItem.objPreloadImage.on('error', function () {
            const objImage = jQuery(this);
            const itemIndex = objImage.data('itemIndex');
            const objItem = g_arrItems[itemIndex];
    ...
    function preloadNearBigImages(objItem) {
        ...
        const currentIndex = objItem.index;
        const lastItemIndex = currentIndex - 1;
    ...
    function onSliderMouseLeave(event) {
        if (g_options.gallery_pause_on_mouseover == true && g_temp.isPlayMode == true && g_objSlider && g_objSlider.isSlideActionActive() == false) {
            const isStillLoading = g_objSlider.isCurrentSlideLoadingImage();
    ...
    function onKeyPress(event) {
        const obj = jQuery(event.target);
        if (obj.is('textarea') || obj.is('select') || obj.is('input')) { return (true); }

        const keyCode = (event.charCode) ? event.charCode : ((event.keyCode) ? event.keyCode : ((event.which) ? event.which : 0));
    ...
    function onGalleryResized() {
        ...
        if (objSize.width != g_temp.lastWidth || objSize.height != g_temp.lastHeight) {
            ...
            if (g_temp.funcCustomHeight) {
                const newHeight = g_temp.funcCustomHeight(objSize);
                if (newHeight) {
                    g_objWrapper.height(newHeight);
                    heightWasSet = true;
                }
    ...
    function onFullScreenChange() {
        const isFullscreen = g_functions.isFullScreen();
        const event = isFullscreen ? t.events.ENTER_FULLSCREEN : t.events.EXIT_FULLSCREEN;

        const fullscreenID = g_functions.getGlobalData('fullscreenID');
    ...
    function initEvents() {
        ...
        g_objWrapper.on('dragstart', (event) => {
            event.preventDefault();
        });
        ...
        if (g_objSlider) {
            jQuery(g_objSlider).on(g_objSlider.events.ITEM_CHANGED, () => {
                const sliderIndex = g_objSlider.getCurrentItemIndex();
            ...
            if (g_options.gallery_pause_on_mouseover == true) {
                const sliderElement = g_objSlider.getElement();
                sliderElement.hover(onSliderMouseEnter, onSliderMouseLeave);

                // on full screen, run on mouse leave
                g_objGallery.on(t.events.ENTER_FULLSCREEN, () => {
                    onSliderMouseLeave();
                });
            }
     ...
     */
    this.getSize = function () {
        const objSize = g_functions.getElementSize(g_objWrapper);
    ...
    this.getNextItem = function (index, forceCarousel) {
        ...
        const objItem = g_arrItems[nextIndex];
    ...
    this.getPrevItem = function (index) {
        ...
        const objItem = g_arrItems[prevIndex];
    ...
    this.setFullScreenToggleButton = function (objButton) {
        ...
        g_objGallery.on(t.events.EXIT_FULLSCREEN, () => {
    ...
    this.setPlayButton = function (objButton) {
        ...
        g_objGallery.on(t.events.START_PLAY, () => {
            objButton.addClass('ug-stop-mode');
        });

        g_objGallery.on(t.events.STOP_PLAY, () => {
    ...
    this.setTextContainers = function (objTitle, objDescription) {
        g_objGallery.on(t.events.ITEM_CHANGE, () => {
            const objItem = t.getSelectedItem();
    ...
    function advanceNextStep() {
        const timeNow = jQuery.now();
        const timeDiff = timeNow - g_temp.playTimeLastStep;
        g_temp.playTimePassed += timeDiff;
        g_temp.playTimeLastStep = timeNow;

        // set the progress
        if (g_temp.objProgress) {
            const percent = g_temp.playTimePassed / g_options.gallery_play_interval;
    ...
    this.startPlayMode = function () {
        ...
        if (g_temp.objProgress) {
            const objElement = g_temp.objProgress.getElement();
    ...
    this.shuffleItems = function () {
        ...
        for (const index in g_arrItems)		// fix index
    ...
    this.selectItem = function (objItem, role) {
        ...
        if (g_temp.isPlayMode == true) {
            ...
            const stillLoading = g_objSlider.isCurrentSlideLoadingImage();
    ...
    this.nextItem = function () {
        let newItemIndex = g_selectedItemIndex + 1;
    ...
    this.toFullScreen = function () {
        ...
        const divGallery = g_objWrapper.get(0);

        const isSupported = g_functions.toFullscreen(divGallery);
    ...
    this.ajaxRequest = function (action, data, isJSON, successFunction) {
        ...
        const urlAjax = g_options.gallery_urlajax;
    ...
    this.requestNewItems = function (catID, isForce, cacheID) {
        let checkCache = g_options.gallery_enable_cache;
        ...
        if (checkCache == true && g_objCache.hasOwnProperty(cacheID)) {
            const htmlItems = g_objCache[cacheID];

            t.changeItems(htmlItems, cacheID);
        } else {
            g_objGallery.trigger(t.events.GALLERY_BEFORE_REQUEST_ITEMS);

            t.ajaxRequest('front_get_cat_items', { catid: catID }, true, (response) => {
                const htmlItems = response.html;
    ...
    this.run = function (galleryID, objParams) {
        const debug_errors = g_options.gallery_debug_errors;
        ...
        if (g_options.gallery_debug_errors == true) {
            try {
                ...
                if (typeof objError === 'object') {
                    ...
                    const lineNumber = objError.lineNumber;
                    const fileName = objError.fileName;
                    const stack = objError.stack;
...
function UGLightbox() {
    ...
    function handlePanelHeight(fromWhere) {
        ...
        const panelHeight = g_objTopPanel.height();
        ...
        const objTextPanelSize = g_objTextPanel.getSize();

        const textPanelHeight = objTextPanelSize.height;
    ...
    function positionTextPanelWide(size) {
        ...
        const textWidth = g_options.lightbox_textpanel_width;
        const minPaddingLeft = 47;
        const minPaddingRight = 40;
        const maxTextPanelWidth = size.width - minPaddingLeft - minPaddingRight;
    ...
    function handleCompactHeight(objImageSize) {
        ...
        const wrapperSize = g_functions.getElementSize(g_objWrapper);
        const textPanelSize = g_objTextPanel.getSize();

        if (textPanelSize.width == 0 || textPanelSize.height > 120) { return (false); }

        if (!objImageSize) {
            const objImage = g_objSlider.getSlideImage();
        ...
        const totalBottom = objImageSize.bottom + textPanelSize.height;

        if (totalBottom < wrapperSize.height) { return (false); }

        const sliderOptions = g_objSlider.getOptions();

        const imagePaddingBottom = textPanelSize.height;
    ...
    function handleCompactTextpanelSizes(showTextpanel) {
        const wrapperSize = g_functions.getElementSize(g_objWrapper);
        const objImage = g_objSlider.getSlideImage();
        const objImageSize = g_functions.getElementSize(objImage);
        ...
        let textPanelWidth = objImageSize.width;

        if (g_objNumbers) {
            const objNumbersSize = g_functions.getElementSize(g_objNumbers);
            textPanelWidth -= objNumbersSize.width;

            // place numbers object
            const numbersLeft = objImageSize.right - objNumbersSize.width;
        ...
        const isChanged = handleCompactHeight(objImageSize);
    ...
    function isSliderImageInPlace() {
        ...
        const isImageInPlace = (g_objSlider.isCurrentImageInPlace() == true);
    ...
    function positionArrowsInside(toShow, isAnimation) {
        ...
        const isImageInPlace = isSliderImageInPlace();
        ...
        if (isImageInPlace == false) {
            ...
            const objImage = g_objSlider.getSlideImage();
            const objImageSize = g_functions.getElementSize(objImage);
            const objSliderSize = g_functions.getElementSize(g_objSlider.getElement());
    ...
    function positionCloseButton(toShow, isAnimation) {
        ...
        const minButtonTop = 2;
        const maxButtonLeft = g_functions.getElementRelativePos(g_objButtonClose, 'right', 2, g_objWrapper);
        ...
        if (isImageInPlace == false) {	// put image to corner
            ...
            const objImageSize = g_functions.getElementSize(objImage);
            const objSliderSize = g_functions.getElementSize(g_objSlider.getElement());
            const objButtonSize = g_functions.getElementSize(g_objButtonClose);
     ...
     */
    function positionElements() {
        const size = g_functions.getElementSize(g_objWrapper);
        ...
        const sliderHeight = size.height;
        const sliderTop = 0;
        const sliderLeft = 0;

        if (g_objSlider) {
            if (g_objTopPanel) {
                const topPanelHeight = g_objTopPanel.height();
                const objOptions = {
    ...
    function isMouseInsideImage() {
        ...
        const isMouseInside = g_objSlider.isMouseInsideSlideImage(obj);
    ...
    function isArrowsHidden() {
        ...
        const opacity = g_objArrowLeft.css('opacity');
    ...
    function updateNumbersText(currentItem) {
        ...
        const numItems = g_gallery.getNumItems();
        const numCurrentItem = currentItem.index + 1;
    ...
    function onZoomChange() {
        ...
        if (g_temp.isCompact == true) {
            const isImageInPlace = (g_objSlider.isCurrentSlideType('image') && g_objSlider.isCurrentImageInPlace() == true);
            if (isImageInPlace == false) {
                hideTextPanel();
                hideNumbers();
            } else {
    ...
    function onSliderAfterReturn() {
        ...
        const isChanged = handleCompactHeight();
    ...
    function onSliderTransitionEnd() {
        const sliderOptions = g_objSlider.getOptions();
        const imagePaddingTop = sliderOptions.slider_image_padding_top;
    ...
    function onSliderClick(data, event) {
        ...
        if (g_options.lightbox_close_on_emptyspace == true) {
            const isInside = g_objSlider.isMouseInsideSlideImage(event);
    ...
    function onPlayVideo() {
        if (g_objTopPanel) {
            hideTopPanel();
        } else if (g_objNumbers) { g_objNumbers.hide(); }
    ...
    function onStopVideo() {
        ...
        } else if (g_objNumbers) { g_objNumbers.show(); }
     ...
     */
    function initEvents() {
        g_objOverlay.on('touchstart', (event) => {
            event.preventDefault();
        });

        g_objOverlay.on('touchend', (event) => {
            t.close('overlay');
        });
        ...
        if (g_objSlider) {
            ...
            // on slider video
            const objVideo = g_objSlider.getVideoObject();
    ...
    this.open = function (index) {
        ...
        setTimeout(() => { g_temp.isRightNowOpened = false; }, 100);
    ...
    this.close = function (fromWhere) {
        ...
        if (slideType != 'image') { g_objWrapper.hide(); } else {
            g_objWrapper.fadeTo(g_temp.fadeDuration, 0, () => {
                g_objWrapper.hide();
            });
        }
    ...
    this.putHtml = function () {
        ...
        const isSmallWindow = g_gallery.isSmallWindow();
...
function UGCarousel() {
    ...
    function setHtml(objParent) {
        ...
        const html = "<div class='ug-carousel-wrapper'><div class='ug-carousel-inner'></div></div>";
        g_objWrapper.append(html);
    ...
    function run() {
        ...
        // set wrapper width
        let realWidth = g_functions.getSpaceByNumItems(numTiles, g_temp.tileWidth, g_options.carousel_space_between_tiles);
        ...
        if (g_temp.isFirstTimeRun == true) {
            ...
            jQuery.each(g_arrItems, (index, item) => {
     ...
     */
    function getTiles() {
        const objTiles = g_objInner.children('.ug-thumb-wrapper');
    ...
    function getNumTilesInSpace(space) {
        const numItems = g_functions.getNumItemsInSpace(space, g_temp.tileWidth, g_options.carousel_space_between_tiles);
    ...
    function getTile(numTile) {
        ...
        const objTiles = getTiles();
        const objTile = jQuery(objTiles[numTile]);
    ...
    function getTileClones(objTile, numClones, dir) {
        let index = objTile.data('index');
        if (index == undefined) {
            throw new Error('every tile should have index!');
        }

        const arrClonedItems = [];
        ...
        for (let i = 0; i < numClones; i++) {
            ...
            const clonedTile = itemToAdd.objTileOriginal.clone(true, true);
    ...
    function getRemainSpaceRight() {
        const wrapperSize = g_functions.getElementSize(g_objCarouselWrapper);
        const innerSize = g_functions.getElementSize(g_objInner);

        const spaceTaken = innerSize.width - wrapperSize.width + innerSize.left;
        const spaceRemain = g_temp.sideSpace - spaceTaken;
    ...
    function getCarouselWidth() {
        const objSize = g_functions.getElementSize(g_objCarouselWrapper);
        return (objSize.width);
    }
    ...
    function getNumTilesInCarousel() {
        ...
        const numTiles = getNumTilesInSpace(width);
    ...
    function positionTiles(setHeight) {
        ...
        jQuery.each(objTiles, (index, objTile) => {
            ...
            const tileSize = g_functions.getElementSize(objTile);
    ...
    function addTiles(numTiles, dir) {
        ...
        const clonesType = (dir == 'left') ? 'prev' : 'next';

        const arrNewTiles = getTileClones(anchorTile, numTiles, clonesType);

        jQuery.each(arrNewTiles, (index, objTile) => {
    ...
    function scrollToTile(numTile, isAnimation, isShort) {
        ...
        const objTile = getTile(numTile);
        const objSize = g_functions.getElementSize(objTile);
        const posScroll = -objSize.left + g_options.carousel_padding;
        ...
        if (isAnimation === true) {
            let duration = g_options.carousel_scroll_duration;
            let easing = g_options.carousel_scroll_easing;
    ...
    function getNeerestTileNum() {
        const innerPos = -getInnerPos();
        const numTiles = getNumTilesInSpace(innerPos);
        const tile1Pos = g_functions.getElementSize(getTile(numTiles)).left;
        const tile2Pos = g_functions.getElementSize(getTile(numTiles + 1)).left;

        if (Math.abs(tile1Pos - innerPos) < Math.abs(tile2Pos - innerPos)) { return (numTiles); }
        return (numTiles + 1);
    }
    ...
    function scrollToNeerestTile() {
        const tileNum = getNeerestTileNum();
    ...
    function fillSidesWithTiles() {
        const spaceLeft = getRemainSpaceLeft();
        const spaceRight = getRemainSpaceRight();
    ...
    function onTouchMove(event) {
        ...
        const diff = g_temp.lastMousePos - g_temp.startMousePos;
        let innerPos = g_temp.startInnerPos + diff;
        const direction = (diff > 0) ? 'prev' : 'next';
        const innerSize = g_functions.getElementSize(g_objInner).width;
    ...
    this.getEstimatedHeight = function () {
        const height = g_options.tile_height + g_options.carousel_padding * 2;
        return (height);
    };
    ...
    this.scrollRight = function (tilesToScroll) {
        ...
        const numTilesInCarousel = getNumTilesInCarousel();
        if (tilesToScroll > numTilesInCarousel) { tilesToScroll = numTilesInCarousel; }

        let numPrev = g_temp.numCurrent - tilesToScroll;
    ...
    this.scrollLeft = function (tilesToScroll) {
        ...
        let numNext = g_temp.numCurrent + tilesToScroll;
    ...
    this.setPlayPauseButton = function (objButton) {
        ...
        g_objThis.on(t.events.START_PLAY, () => {
            objButton.addClass('ug-pause-icon');
        });

        g_objThis.on(t.events.STOP_PLAY, () => {
            objButton.removeClass('ug-pause-icon');
        });

        g_functions.setButtonOnClick(objButton, () => {
     ...
     */
    function isApproveTileClick() {
        const passedTime = g_temp.lastTime - g_temp.startTime;
        const passedDistanceAbs = Math.abs(g_temp.lastMousePos - g_temp.startMousePos);
...
function UGTabs() {
     ...
     */
    function onTabClick() {
        const classSelected = g_options.tabs_class_selected;

        const objTab = jQuery(this);
        if (objTab.hasClass(classSelected)) { return (true); }

        g_objTabs.not(objTab).removeClass(classSelected);
        objTab.addClass(classSelected);

        const catID = objTab.data('catid');
    ...
    function onSelectChange() {
        const objSelect = jQuery(this);
        const catID = objSelect.val();
...
function UG_API(gallery) {
    ...
    function convertItemDataForOutput(item) {
        ...
        const addData = item.objThumbImage.data();
    ...
    this.on = function (event, handlerFunction, notCache) {
        // remember cache
        if (notCache !== true) {
            g_arrEvents.push({ event, func: handlerFunction });
        }
        ...
        switch (event) {
            case 'item_change':
                ...
                g_objGallery.on(g_gallery.events.ITEM_CHANGE, () => {
                    const currentItem = g_gallery.getSelectedItem();
                    const output = convertItemDataForOutput(currentItem);
    ...
    this.resetZoom = function () {
        const objSlider = g_gallery.getObjSlider();
    ...
    this.getItem = function (numItem) {
        const data = g_gallery.getItem(numItem);
        const output = convertItemDataForOutput(data);
...
if (typeof g_ugFunctions !== 'undefined') { g_ugFunctions.registerTheme('default'); } else { jQuery(document).ready(() => { g_ugFunctions.registerTheme('default'); }); }
...
function UGTheme_default() {
    ...
    function setHtml() {
        ...
        let classButtonFullscreen = 'ug-default-button-fullscreen';
        let classButtonPlay = 'ug-default-button-play';
        let classCaptureButtonFullscreen = '.ug-default-button-fullscreen';
        let classCaptureButtonPlay = '.ug-default-button-play';
     ...
     */
    function initPanel() {
        const objGallerySize = g_gallery.getSize();
        const galleryWidth = objGallerySize.width;
        ...
        // set panel size
        const objStripPanelSize = g_objStripPanel.getSize();
        let panelHeight = objStripPanelSize.height;
        ...
        const stripPanelElement = g_objStripPanel.getElement();
        g_functions.placeElement(stripPanelElement, 'left', 'bottom');

        // init hide panel button
        if (g_objButtonHidePanel) {
            const buttonTip = g_objButtonHidePanel.children('.ug-default-button-hidepanel-tip');
            g_functions.placeElement(buttonTip, 'center', 'middle');

            // set opacity and bg color from the text panel
            if (g_objTextPanel) {
                const objHideButtonBG = g_objButtonHidePanel.children('.ug-default-button-hidepanel-bg');

                const hidePanelOpacity = g_objTextPanel.getOption('textpanel_bg_opacity');
                objHideButtonBG.fadeTo(0, hidePanelOpacity);

                const bgColor = g_objTextPanel.getOption('textpanel_bg_color');
                objHideButtonBG.css({ 'background-color': bgColor });
            }
        }

        // position buttons on the text panel:
        let paddingPlayButton = 0;
        let panelButtonsOffsetY = 0;
        if (g_objButtonHidePanel) {
        ...
        if (g_objButtonPlay) {
            let buttonPlayOffsetY = panelButtonsOffsetY;
        ...
        if (g_objButtonHidePanel) {
            ...
            { g_functions.placeElement(g_objButtonHidePanel, 'left', 'top'); } else {		// place above the strip panel
                const stripPanelHeight = stripPanelElement.outerHeight();
    ...
    function placePanel() {
        if (g_temp.isPanelHidden || isPanelNeedToHide() == true) {
            // place panel hidden
            const newPanelPosY = getHiddenPanelPosition();
    ...
    function placeSlider() {
        const sliderTop = 0;
        const sliderLeft = 0;
        const galleryHeight = g_gallery.getHeight();
        let sliderHeight = galleryHeight;

        if (g_objStripPanel && isPanelHidden() == false) {
            const panelSize = g_objStripPanel.getSize();
            sliderHeight = galleryHeight - panelSize.height;
        }

        const sliderWidth = g_gallery.getWidth();
    ...
    function isPanelNeedToHide() {
        ...
        const windowWidth = jQuery(window).width();
        const hidePanelValue = g_options.theme_hide_panel_under_width;
    ...
    function checkHidePanel() {
        ...
        const needToHide = isPanelNeedToHide();
    ...
    function getHiddenPanelPosition() {
        const galleryHeight = g_objWrapper.height();
        let newPanelPosY = galleryHeight;
        if (g_objButtonHidePanel) {
            const objButtonSize = g_functions.getElementSize(g_objButtonHidePanel);
    ...
    function showPanel(noAnimation) {
        ...
        const galleryHeight = g_objWrapper.height();
        const panelHeight = g_objPanel.outerHeight();

        const newPanelPosY = galleryHeight - panelHeight;
...
if (typeof g_ugFunctions !== 'undefined') { g_ugFunctions.registerTheme('tilesgrid'); } else { jQuery(document).ready(() => { g_ugFunctions.registerTheme('tilesgrid'); }); }
...
function UGTheme_tilesgrid() {
    ...
    let g_gallery = new UniteGalleryMain(),
        ...
        g_objects,
        g_objWrapper;
    let g_objThumbsGrid = new UGThumbsGrid(),
    ...
    function getGalleryWidth() {
        let galleryWidth = g_gallery.getSize().width;
    ...
    function setHtml() {
        ...
        if (g_options.theme_navigation_type == 'bullets') {
            ...
            const numPanes = g_objThumbsGrid.getNumPanesEstimationByWidth(galleryWidth);

            const objOptions = g_gallery.getOptions();
        ...
        if (g_options.theme_navigation_type == 'arrows') {
            let htmlAdd = "<div class='ug-tile-navigation-wrapper' style='position:absolute'>";
    ...
    function getHeightEstimation(galleryWidth) {
        ...
        let gridHeight = g_objThumbsGrid.getHeightEstimationByWidth(galleryWidth);
        const numPanes = g_objThumbsGrid.getNumPanesEstimationByWidth(galleryWidth);
    ...
    function actualRun() {
        ...
        const totalHeight = getHeightEstimation(galleryWidth);
    ...
    function positionElements() {
        const objGallerySize = g_gallery.getSize();
        let g_objGridSize = g_objThumbsGrid.getSize();

        const gridElement = g_objThumbsGrid.getElement();
        g_functions.placeElement(gridElement, g_options.theme_grid_align, 0);

        g_objGridSize = g_objThumbsGrid.getSize();

        let galleryHeight = g_objGridSize.height;

        const numPanes = g_objThumbsGrid.getNumPanes();
        ...
        if (numPanes > 1) {
            ...
            if (g_objBullets) {
                ...
                const bulletsElement = g_objBullets.getElement();
                const bulletsWidth = g_objBullets.getBulletsWidth();

                // get bullets offset x (relative to the grid)
                const bulletsX = g_objGridSize.left + g_functions.getElementRelativePos(bulletsWidth, g_options.theme_navigation_align, g_options.theme_navigation_offset_hor, gridElement);

                g_functions.placeElement(bulletsElement, bulletsX, galleryHeight + g_options.theme_bullets_margin_top);

                const sizeBullets = g_functions.getElementSize(bulletsElement);
            ...
            if (g_objNavWrapper) {
                ...
                const navX = g_objGridSize.left + g_functions.getElementRelativePos(g_objNavWrapper, g_options.theme_navigation_align, g_options.theme_navigation_offset_hor, gridElement);

                g_functions.placeElement(g_objNavWrapper, navX, galleryHeight + g_options.theme_arrows_margin_top);

                const sizeNav = g_functions.getElementSize(g_objNavWrapper);
    ...
    function onTileClick(data, objTile) {
        ...
        const objItem = g_objTileDesign.getItemByTile(objTile);
        const index = objItem.index;
